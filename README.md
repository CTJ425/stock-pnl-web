...
> **目前版本：0.6.4-dev.5**（版本號顯示於畫面左下角徽章）
...
修 dev.1 在測試區實測到的死結：ETF 不在月營收報表內、缺口永遠填不滿，
於是把「最新那幾個月」永遠釘在待抓清單上，真正的公司再也拿不到更舊的資料
（實測第 2 輪起每次都重抓同樣四個月、一筆也補不進去）。

改為在檔案裡記錄「已經找到哪個月份為止」，區分**還沒去找**與**找過了就是沒有**。
ETF 現在三輪之後就收斂，不再拖累其他標的。

#### dev.3（2026-07-28）

修一個還沒發作的漏洞：每晚的批次是**整份重建** `fundamental/{ticker}.json`，
而 dev.2 新增的「已找到哪個月為止」沒被帶過去。等於每個交易日的第一輪
都會把回補進度抹掉、隔天再把 12 個月重走一遍，「補滿後零成本」形同虛設。

順手把建檔與註記判斷抽成純函式並補上測試 —— 這類欄位遺漏在原本的位置
既沒有型別檢查也沒有測試碰得到，同一種錯誤已經出現兩次。

#### dev.4（2026-07-28）

回補上線後遇到「畫面看起來還是舊的，但分不出是資料問題還是畫面問題」，
補兩個東西讓它變得可判斷、可自救：

- **基本面分頁標示資料產出時間**：「資料更新於 07-28 10:25（共 12 個月）」。
  與估值的「資料日」是兩件事 —— 那個是資料自己宣告的日期，這個是我們寫檔的時刻。
  少了幾個月時，一眼就分得出是「資料就這樣」還是「你看到的是舊的一份」。
- **個股分析頁加「重新整理」鈕**：強制重抓籌碼、技術面與基本面，
  不必整頁重載、也不必切換股票。

#### dev.5（2026-07-28）

**上一版加的時間戳立刻抓到真兇。** 一般視窗顯示「資料更新於 07-27 20:15（共 1 個月）」，
無痕視窗同一頁是 12 個月 —— 瀏覽器拿的是一份**舊了 15 小時**的檔案。

原因：盤後批次寫檔時沒有指定 `cacheControl`，SDK 預設 3600，
Storage 因此回 `cache-control: public, max-age=3600`。最惡劣的是**使用者救不了自己**：
`Ctrl+Shift+R` 只跳過文件與其子資源的快取，**不涵蓋 JS 之後才發出的 `fetch()`**，
所以怎麼硬重整都沒用，只有無痕視窗（空快取）才對。

兩邊一起修：前端讀取一律 `cache: 'no-store'`；後端寫檔改為不快取。
前端那道不能省 —— 既有檔案要等下次寫入才會換掉 metadata。
