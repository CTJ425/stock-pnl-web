...
> **目前版本：0.6.9-dev.5**（版本號顯示於畫面左下角徽章）
...
- 修好 **AI 分析寫到一半被截斷**（只剩前一兩段加一行「未寫完」）。
  原因是呼叫 OpenAI 相容端點時沒有指定輸出上限，用的是端點預設值 ——
  很多端點預設只有幾百 token。現在會明確送出上限。

#### dev.4

- **AI 分析現在推理型模型也能用**（deepseek-r1 / qwq / gpt-oss 等），不必再為此換模型：
