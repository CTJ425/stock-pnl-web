...
> **目前版本：v0.3.7-dev.5**（dev 分支開發中；併入 main 後為 v0.3.7。版本號顯示於畫面左下角徽章與「服務狀態」頁）
...
- **服務狀態**：提供 GitHub Status 風格的頁面，檢視各項系統健康狀況、快取狀態與歷史檢測紀錄。
- **版本標籤**：版本與作者顯示於服務狀態頁，方便回報問題時確認版本。
...
### v0.3.7（開發中 · dev：`0.3.7-dev.5`；併入 main 後定為 v0.3.7）

**dev.5 — 新增「基本面」分頁（EPS 與估值指標）**
- **個股分析頁新增第四個分頁籤「基本面」**：本益比、殖利率、股價淨值比、財報單季 EPS、營收與淨利。
  EPS 是公司獲利÷股數，和籌碼（法人買賣股數）是完全無關的兩種資料，故獨立成頁而非併進籌碼報告。
- **EPS 取自官方財報**：TWSE 綜合損益表的「基本每股盈餘」。該端點按產業分五張表
  （一般業/金控/保險/證券期貨/異業），必須全抓合併 —— 金控股不在一般業表裡。
- **新增 `stock_fundamentals` 資料表**：以 (代號, 年度, 季別) 為主鍵、**不設保留期**。
  官方端點沒有季別參數、只回最新一期，逐季趨勢只能靠本表累積；累積到兩季以上就會出現 EPS 走勢圖。
- **每季只抓一次**：依台灣財報申報期限（Q1 5/15、Q2 8/14、Q3 11/14、年報 3/31）推算應公布的最新一季，
  已在庫就完全不發外部請求。實測第二次批次由 10.1 秒降到 1.9 秒。
- **ETF 有專屬文案**：ETF 沒有 EPS 也沒有本益比（它不做生意、價值來自持有的一籃子股票），
  不顯示籠統的「查無」。上櫃與美股不支援，另有各自的說明。
- ⚠️ 需重跑 `sources/supabase/schema.sql` 第 7 段（建 `stock_fundamentals`）並重新部署 `stock-report`。

**dev.4 — 籌碼逐日檢視 + 法人並排比較**
- **三大法人表格可回看 7 天**：區塊標題旁新增日期鈕（07/16 … 07/24 最新），點了就切換整張表格。
  「連買連賣」改為**算到所看那一天為止**的連續天數（原本只算最新日），由前端依 history 計算。
- **買賣超圖新增「全部（並排）」**：四個法人同時並排比較，各自一個類別色，右側空白處附**圖例**
  標明哪個顏色是哪個法人，並顯示最近交易日的約當張數。hover 一次列出當日四個法人的數字。
- 顏色的分工講清楚了：單一法人時顏色表達漲跌（紅正綠負），並排時顏色表達身分、正負改看零軸上下。
  兩者不會疊在一起。合計不與其組成並排（它就是四項相加，畫進去等於重複計算）。

**dev.3 — 個股分析頁 + 籌碼走勢圖（盤後籌碼報告 v2）**
- **改為獨立「個股分析頁」**：庫存總覽台股每列的按鈕改為「分析」，點擊下鑽到專屬頁面（不再用彈出視窗），內含 `籌碼 / 技術面 / 我的持股` 三個分頁籤。技術面（日線 / 週線 / 季線）為佔位頁，需另存歷史股價後才會接上。
- **三大法人拆出買進 / 賣出**：不再只顯示買賣超。表格改為 買進 / 賣出 / 買賣超 / 約當張數 / **連買連賣**，列出 外資 / 外資自營商 / 投信 / 自營商 / 三大法人合計。資料源不變（T86 同一份回應本就含這些欄位，先前被丟棄）。
- **融資融券拆出買進 / 賣出 / 償還並可看走勢**：改用帶 `date` 參數的 TWSE rwd 端點（舊的 OpenAPI 端點沒有 date，只能取當天，這是先前做不出走勢圖的原因），新增 買進 / 賣出 / 償還 / 較前日 / **連增連減**；舊端點保留為當日備援。
- **近 7 日走勢圖（自繪 SVG，不引入圖表函式庫）**：買賣超長條圖（可切換法人）＋ 融資、融券餘額折線圖（兩者量級差距大，各自獨立縱軸）。支援 hover 看數值，缺資料的日子斷線不內插。
- **伺服器只回結構化資料**：`stock-report` 不再產生 HTML（刪除 `reportHtml.ts`），畫面全部由 React 繪製，Storage 報告體積約砍半。報告內嵌最近 7 個交易日 `history`；單次呼叫最多回補 5 個缺漏日，不足時照常出圖並在說明中標示「歷史資料回補中」。
- **標明資料出處**：籌碼報告表頭顯示「資料日期 2026-07-24（最近交易日盤後）· 報告更新時間 2026-07-25 16:13」——
  前者是資料涵蓋到哪個交易日，後者是這份報告實際產生的時間（兩者常差一天）。表頭位於 PDF 擷取範圍內，
  下載的 PDF 自己看得出是哪支股票、哪一天、什麼時候產的。
- **PDF 仍輸出淺色文件**：擷取前動態套上 `.report-surface` 淺色容器，深色主題下載的 PDF 一樣像一份文件。
- ⚠️ **需重新部署 `stock-report`**：舊版函數回的是 schema 1（HTML），新前端會顯示「伺服器回傳的報告格式不符」。無需 schema migration（新的 `MI_MARGN_D` dataset 沿用現有 `chip_raw_cache`）。
