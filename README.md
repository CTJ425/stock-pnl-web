...
> **目前版本：v0.2.3.1**（版本號固定顯示於應用畫面左下角，如 `v0.2.3.1 | Ivan Chen`）
...
> 第四段版號（如 `v0.2.3.1`）代表欄位增減、文案這類不動計算邏輯的小調整。

### v0.2.3.1（2026-07-20）
- **庫存總覽新增「投入成本」欄**：目前持股當初投入的金額（平均買入成本 × 持有股數，含買進手續費），也就是「現在還壓在裡面」的錢；已賣出的部分不計入。與「目前市值」相鄰，可直接對照市值與未實現損益。金額同樣採「含費 / 未含費」雙行。
