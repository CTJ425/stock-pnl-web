...
> **目前版本：0.3.8-dev.2**（dev 分支開發中；併入 main 後為 0.3.8。版本號顯示於畫面左下角徽章）
...
### 0.3.8（開發中 · dev：`0.3.8-dev.2`；併入 main 後定為 0.3.8）

**dev.2 — 修正夜間排程的 pg_net 逾時**
- `schema.sql` 的 cron 呼叫補上 `timeout_milliseconds := 60000`。pg_net 預設只有 **5000ms**，
  但每天第一次執行要抓當天的 T86 與融資融券大檔，實測需 **10–13 秒**（之後快取命中約 2 秒）。
- 影響不是「排程壞掉」—— 實測即使客戶端逾時，Edge Function 仍會在伺服器端跑完、報告正常寫入。
  真正的損失是**可觀測性**：`net._http_response` 每晚都記成 `status_code = null` 的逾時，
  導致「逾時但成功」與「真的失敗」無法區分。修正後可正常記錄 `200` 與完整回應內容。

**dev.1 — 個股分析獨立成頁、移除服務狀態**
- **個股分析改為導覽分頁**：不再是從庫存總覽下鑽的檢視。頁內用**下拉選單切換個股**，
  看另一檔不必返回再點 —— 選單只列台股持股（盤後籌碼只涵蓋上市台股，不放點了沒用的選項）。
  庫存總覽因此移除「個股分析」欄，回到純持股表。
- 未設定 Supabase（本機模式）時整個分頁隱藏，與盤後報告一路以來的入口規則一致。
- **移除「服務狀態」功能**：整頁與其健檢邏輯（`serviceHealth`）皆刪除。
  原本頁內的 GitHub Repository 連結改置於**頁尾免責聲明下方**。
- 內部整理：庫存總覽的 `buildRows` 抽成共用的 `utils/holdingRows.ts`，
  讓兩頁的持股損益計算是同一份，不會走鐘。
