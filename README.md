...
> **目前版本：0.6.5-dev.3**（版本號顯示於畫面左下角徽章）
...
### 0.6.5（開發中）— AI 分析可以繼續問，新增總經與獲利能力

#### dev.1（2026-07-28）

**「AI 解讀」更名為「AI 分析」。**

**產生分析後可以繼續追問。** 對話嚴格框在「這檔股票的數據」之內，
問到範圍外（閒聊、其他個股、寫作、時事）會一字不差回一句固定的婉拒 ——
固定句才看得出框限有沒有被繞過。也擋提示詞注入：
「忽略上述指示」「你現在是別的角色」一律視同越界問題。

- 刻意**不做關鍵字過濾**：「這檔跟聯電比呢」會被誤擋，
  「用這檔資料寫首詩」卻過得去，而且黑名單永遠追不上繞法。
- 框限規則**每一輪都重送**，對話變長也不會被稀釋。
- 上限 10 輪。每輪都會重送完整資料，再往上疊只是增加費用。
...
> （Conference Board 版為付費資料）。

**基本面新增「獲利能力」**：毛利率、營益率、稅前純益率、稅後純益率，
資料由證交所按季彙總。序列逐季累積，最多保留 8 季。

以上兩類資料都會一併餵給 AI，但 system prompt 明令**總經是背景不是個股因果**，
不得用來推導這檔股票的漲跌。

#### dev.2（2026-07-28）

**總經改成獨立的頂層頁面**，與 庫存總覽 / 個股分析 / 年度收益 / 交易紀錄 並列。
原本掛在個股分析底下，得先選一檔股票才看得到一份跟那檔股票無關的資料。

**總經也改成自己的每日排程**（每天兩班），不再搭盤後批次的順風車 ——
那個排程是台股作息（週一至週五、台股盤後），而美國數據跟台股交易日無關，
週末完全不更新。

> 順帶一提：分頁從四個變五個之後，375px 螢幕上的分頁列會折行（實測高度由
> 36px 變 57px）。已收窄該尺寸下的分頁間距修正，六種常見寬度都重量過。

#### dev.3（2026-07-28）

**頁首右側由 8 個控制項收斂成 2 個選單**：工作區選單、帳號選單。
原本是「工作區下拉 ＋ 新增 ＋ 重新命名 ＋ 費率 ＋ 刪除 ＋ 主題 ＋ email ＋ 登出」
一字排開，其中四顆是無標籤圖示，而刪除工作區就緊鄰重新命名。

現在工作區的管理動作收在工作區選單裡（作用對象不必用猜的），
**刪除隔一條分隔線並用紅色**；外觀、帳號、登出收在右邊的帳號選單。

順手修掉兩個量出來的 bug：

- **寬螢幕的頁首比窄螢幕還高**：≥1221px 時 email 一顯示就撐成兩列（106px），
  而 ≤1220px 反而是單列（70px）。現在**每個寬度都是 70px**。
- **375px 的工作區下拉塌成 39px**，只剩一個箭頭，看不出目前在哪個工作區 ——
  而工作區決定畫面上每一個數字。現在名稱穩定可讀。
