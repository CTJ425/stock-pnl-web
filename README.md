...
> **目前版本：0.6.8-dev.1**（版本號顯示於畫面左下角徽章）
...
### 0.6.8（開發中）— 個股分析改為單頁、折線圖改成 Google Finance 風格

#### dev.1

- **折線圖新樣式**（匯率頁 2 張、籌碼頁融資／融券 2 張）：
  - 線下方**漸層面積填充**，往下淡出。
  - hover 時出現**垂直虛線**，並在該點畫實心圓。
  - **提示框貼著資料點**上下移動，不再固定在圖表頂端；最左／最右的點也不會超出容器。
  - 資料點超過 20 個時不再逐點畫圓（一年 260 顆圓點會把線糊成一條毛毛蟲），
    籌碼頁的 7 天則維持逐點顯示。
- **基本面的月營收新增走勢圖**（表格上方），營收缺漏的月份斷線不內插。
- K 線、成交量長條、KD 三張圖維持原樣。
- **個股分析改為單一長頁**：籌碼、基本面、技術面、我的持股不再是四個分頁，
  改成一頁到底（順序：我的持股 → 籌碼 → 基本面 → 技術面），每段一張卡片。
  AI 分析仍是獨立分頁。
  - 「下載 PDF」改為匯出籌碼＋基本面＋技術面，**不含持股數字**。
  - 圖表改為整張圖一個鍵盤焦點、方向鍵逐點移動（原本每個資料點都是一個 Tab 停留點，
    一年份的日 K 就有 244 個）。
