...
### 0.6.31（2026-08-05）— 大盤法人買賣超補上逐日表格、列出自己的交易；年度收益加報酬率
...
- **同一張表右側併入自己的台股買賣金額**（我的買進 / 我的賣出，單位是元）：
  「法人這幾天在買還是在賣」與「那我呢」是同一個問題的兩半，分成兩張表就得自己對日期。
  買進為實付金額（含手續費）、賣出為實收金額（已扣手續費與交易稅），美股交易不計入。
  只涵蓋表中那 7 個交易日，沒下單的日子顯示「—」。
