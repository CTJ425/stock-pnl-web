...
> **目前版本：0.6.7-dev.1**（版本號顯示於畫面左下角徽章）
...
- **新增「外幣匯率」頂層分頁**，以台幣為本位，支援美元、日圓、歐元、人民幣、港幣、英鎊、澳幣、韓元 8 種外幣。
  - 8 張幣別卡顯示「1 單位外幣可換多少台幣」與當日升貶方向；點卡片切換，選擇會記住。
  - **雙向換算器**：台幣與外幣兩個欄位都可輸入，另一邊即時換算。
  - **走勢圖**可切 3 個月 / 6 個月 / 1 年，並顯示區間高低與漲跌幅。
- 資料由新的每日排程 `fx-daily`（台北 11:00 / 17:00）預產於 `fx/twd.json`，前端直接讀 Storage。
- ⚠️ **匯率為市場中價，非銀行牌告匯率。** 原訂採用台灣銀行牌告匯率，但其 CSV 端點已被
  JS 人機驗證擋住（Edge Function 無法通過），故改用 Yahoo Finance。實際結匯請以往來銀行的
  現金／即期買賣價為準。
- 修掉圖表 Y 軸對小於 1 的數字一律標成「0」的問題（既有圖表數值都 ≥ 1 所以沒踩到，匯率會）。
- 手機版型待桌機功能驗證後再處理；分頁增為六格，底部導覽列（0.6.6）本來就容得下。
