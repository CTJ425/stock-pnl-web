...
- **Status**: ✅ **完成** —— 純前端，不需要部署 Edge Function
- **Agent**: Claude
- **Timestamp**: 2026-08-05 09:25:00 Asia/Taipei
- **需求**：在同一列對照「法人在買賣」與「我在買賣」，不必切到交易紀錄頁自己對日期。
- **欄位**：Task 62 的表格右側加「我的買進（元）」「我的賣出（元）」，跟著同一個 7 交易日視窗。
- **金額定義**沿用交易紀錄頁 `cashFlow`：買進含手續費（實付）、賣出扣手續費與交易稅（實收）。
- **單位刻意不統一**（法人億元、自己元）：個人單筆換算成億後整欄都是 0.0；
  靠表頭「（元）」與 `border-left` 分隔線標示換檔。
- **資料傳遞走 prop**（`AppShell` → `MacroPage` → `TwMarketSection`），不在元件內呼叫
  `useWorkspace()` —— 該 hook 無 provider 會 throw，而兩個測試檔都是裸 render。
- 美股交易不計入；沒下單的日子「—」不給 0；自己的金額不套紅綠（無方向語意）。
