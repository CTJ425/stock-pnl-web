...
- Status: **完成 —— 測試區已部署並覆驗（PCE 補上 2026-06、第二次呼叫回 unchanged）；正式區待部署**
...
1. 正式區部署與同一組覆驗（0.6.11 已併入 `main`）。
2. **明天（2026-08-01）21:00 / 23:00 那兩班才是真正的排程迴歸**：
   要看到第二班確實發出請求（而非日期短路），且 `asOf` 只在有新數據時才跳。
3. 下一次值得盯的月度發布是 8 月的 CPI（月中）與 PCE（月底）——
