...
1. **cron 改密尚未執行**（對外操作，需授權）：`0 13,15 * * *` →
   `*/30 12-18 * * *`（台北 20:00–02:30 每 30 分）。
   ⚠️ 要用 `cron.alter_job` 而非重跑 `cron.schedule`（佔位符地雷 BUG-002/003）。
   **不改也能運作**，只是密集度停在現有兩班、追不到 FRED 匯入延遲。
2. **前端 `timeline.ts` 的 `RELEASE_RULE` 尚未改用行事曆**，仍是區間推估。
