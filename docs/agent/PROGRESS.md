...
- Agent: Claude
- Action: 季度獲利能力歷史回補（0.6.21 定版）
- Status: **完成 —— 兩區 schema 與 Edge Function 已更新；786 測試全過**
...
`HoldingProfitSection` 由 `Macro/` 移到 `StockDetail/`（它講的是公司不是市場，
而 `AnalysisPage` 本來就只列台股，正好對上資料的涵蓋範圍）。
`sparkline.ts` 兩個 feature 都要用，故提到共用的 `Charts/`。

### 驗證

`npm test` **786/786**（新增 20 條）、lint 3 個既有 warning、build 通過。
