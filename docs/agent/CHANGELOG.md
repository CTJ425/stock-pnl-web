...
### 0.9.39-dev.2（2026-09-10）— 修復觀察清單搜尋兩個缺陷，並重整字卡版面

- 🐛 **修復加入觀察搜尋把已加入的股票靜默隱藏**：搜尋結果過去會把已在觀察清單中的代號整筆移除且不給任何說明，使用者輸入代號後看到空白清單，得到的訊息是「這檔股票不存在」，事實是「已經加過了」。現在改為列出但停用、標示「已在觀察清單」，並排到可加入項目之後。
- 🐛 **修復部分來源失敗的殘缺台股清單被當成完整清單快取**：上市（TWSE）與上櫃（TPEx）兩個來源過去只要有一個成功就合併回傳，另一個掛掉時會得到半份清單並被快取 30 分鐘，期間該來源的每一檔股票都顯示為「查無此股」。現在任一來源失敗即整體視為失敗，交給既有的後援鏈處理。回 HTTP 200 但內容為空陣列的情況一併視為失敗。
...
- ✅ **測試**：新增 11 條測試，總數 1772 → 1782，全部通過；`npm run build` exit 0。其中空陣列來源那條回歸測試已反向驗證，移除修正後確認轉紅。

### 0.9.39-dev.1（2026-09-10）— 股票搜尋補上 Carbon 載入轉圈動畫
...
- ✅ **測試**：新增 12 條測試，總數 1760 → 1772，全部通過；`npm run build` exit 0。其中兩條回歸測試各自移除對應修正後皆確認轉紅，不是空轉。
