...
### 0.9.70-dev.1（2026-09-25）— 年度收益搜尋時自動展開符合的年度與賣出明細（Task 169）

- 🔍 **年度收益搜尋直接看到明細**：在搜尋框輸入代號或名稱後，符合的年度與有賣出的個股會自動展開，逐筆賣出明細直接顯示，不必再手動點開年度。清空搜尋後恢復收合；搜尋期間仍可手動收合或展開。
- ✅ **驗證**：新增搜尋展開測試（拿掉修正即失敗）；144 個測試檔／2,478 條測試：2,471 通過、7 略過、0 失敗；`npm run build`、`npm run typecheck:edge` exit 0。

### 0.9.69（2026-09-24）— 程式碼健檢後的整理：網址路由、延遲載入、拆大檔、套件升級（Task 168）

- 🔗 **網址記住目前頁面**：切換分頁會寫進網址（例如 `#/transactions`、`#/analysis/2330`）。重新整理會停在原頁、瀏覽器返回鍵可回上一頁、也能直接分享某一頁的連結。密碼重設信的連結不受影響。管理後台重新整理時會等權限判定完成，不再被踢回首頁。
- ⚡ **首頁載入更輕**：個股分析、年度收益、交易紀錄與手續費重算視窗改為點到才載入，首次載入的主程式從 156 KB 降到 112 KB（gzip）。「新增交易」表單維持立即可用。
- 🧱 **拆開大檔（功能不變）**：`AppShell.tsx` 958 → 約 370 行；`stock-report` 主檔 5,487 → 3,829 行，管理後台與 Discord 的處理拆成獨立模組，26 個 action 改用一張路由表，每列寫明權限檢查方式（逐一比對與舊版相同）；`index.css` 5,708 行依功能拆成 13 個檔；TPEx 備援清單改存 JSON，並新增 `npm run refresh:tpex-fallback` 更新腳本（清單已更新到 2026-09-24，共 1,015 檔）。
- 📦 **套件升級**：TypeScript 7、vitest 5、jsdom 30，其餘 minor／patch 全數更新；e2e 工作流程的 Node 版本與 CI 對齊為 24。
- 🧹 **程式碼品質**：oxlint 警告清到 0（原本 45 條，加上升級後新增的規則）；清掉 30 行註解裡的零寬空白；修正 `WorkspaceProvider` 每次重繪都會多建立一個資料提供者；測試改用 threads 執行（約 142 秒，原本 154–161 秒），並在每個測試後卸載元件，避免測試結束後才報錯；修正「盤後探針戰情室」測試每晚 22:30 後必定失敗（改為固定測試時間）；DEV e2e 工作流程補上瀏覽器安裝步驟。
- ✅ **驗證**：144 個測試檔／2,477 條測試：2,470 通過、7 略過、0 失敗；`npm run build`、`npm run lint`、`npm run typecheck:edge` exit 0，GitHub CI 通過。DEV 已部署 `stock-price` 與 `stock-report`，權限檢查冒煙測試全數符合預期，排程 `source-probe` 部署後回應 200。瀏覽器實測延遲載入、重新整理、返回鍵與新增交易，無 console 錯誤。

### 0.9.68（2026-09-24）— 移除 AI 分析、「新增交易」移到頁首、版本號移到頁尾、Discord 持股卡片加上券商口徑（Task 167）

- 🗑️ **移除 AI 分析**：個股頁「AI 分析」分頁與管理後台「AI 設定」一併拿掉，相關前端程式與測試全數刪除；`ai-proxy` Edge Function 下線，資料庫刪除 `app_settings` 資料表與 `get_ai_settings()`。日後要重做可從 git 歷史找回。
- ➕ **「新增交易」移到頁首**：右下角浮動按鈕改為頁首按鈕，不再蓋住右下角的資訊。電腦版顯示圖示與文字；手機版只顯示圖示，排在工作區選單與帳號之間。
- 🏷️ **版本號移到頁尾**：左下角固定的版本徽章拿掉，改接在頁尾免責聲明最後（「…諮詢之用 · 0.9.68」），字色較淡且不會斷行。手機版由頁尾負責讓出底部導覽列的空間。登入頁不再顯示版本號。
- 📒 **Discord 持股卡片加上「券商」**：與庫存總覽的「券商」欄同一算法（台股牌告 0.1425%、不打折）。總計列顯示「券商 ±金額」，每檔持股在未實現後加「（券商 ±金額）」；與實際未實現相同時不重複顯示，美股不適用。頁尾註明「券商＝牌告 0.1425% 未折讓」。
- ✅ **驗證**：142 個測試檔／2,466 條測試：2,459 通過、7 略過、0 失敗；`npm run build`、`npm run lint`、`npm run typecheck:edge` exit 0。Playwright 在 1440／1024／375／320 px 確認頁首無溢出、頁面無橫向捲動、版本號在手機底部導覽列上方，頁首按鈕可開啟新增視窗。

### 0.9.67（2026-09-23）— 自選股產業分組不再來回跳動（BUG-085）

- 🐛 **自選股分組跳動**：報價快取 `price_cache` 原本不存產業別，命中快取時回傳的產業是空的，每 60 秒輪詢時，不在內建字典裡的個股（例如南茂 8150）會在「半導體業」與「其他」之間來回切換。`price_cache` 新增 `industry` 欄位，`stock-price` 寫入快取時一併保存、命中快取時一併回傳。
- 📚 **內建產業字典**：補上南茂 8150（半導體），即時報價尚未提供產業別時也能正確分組。
- ✅ **驗證**：151 個測試檔／2,620 條測試：2,613 通過、7 略過、0 失敗；`npm run build` exit 0。DEV 與 PROD 皆套用 DDL 並重新部署 `stock-price`（兩區 bundle sha 相同），連續兩次查詢 8150 皆回傳「半導體業」（第二次為快取命中），兩區 `verify_setup()` 全數 PASS。

### 0.9.66（2026-09-23）— 全專案稽核修補第 4 批：前端體驗與無障礙（Task 166）

- 📈 **個股頁**：漲停／跌停徽章與分時圖參考線（昨收 ±10% 取到升降單位）；分時與日線標示「快取於 HH:MM」，資料代號與目前個股不符時不顯示；單點資料也會畫出；美股成交量改以「股」計；統計格改為語意化清單；試算欄位逐一驗證並各自提示；價格階梯註明「僅供試算參考，非交易所公告之漲跌停價」。
- 📊 **基本面與籌碼**：季報表改為最近 9 季滾動視窗（移除寫死的 2024-Q1）；近 4 季合計不足時顯示「資料不足 (n/4)」；籌碼空狀態區分尚未公布／無資料／抓取失敗；三大法人合計與分項不符時標示；「資料更新於」固定以台北時間顯示。
- 🌏 **總經與匯率**：平日若最新市場日早於今天即顯示休市；國際指數加請求序號防護並顯示已收盤；市場資料跨分頁共用；市場與外資 TOP 50 區分「尚未產生」與「資料格式不符」；成交筆數單位寫在儲存格內；當日走勢抓取失敗會提示；匯率過期改算交易日（超過 2 個交易日才警示）；匯率頁卸載即中止請求，重新整理同時更新歷史。
- 🧭 **介面與交易**：Token 更新後重查管理員身分；登出失敗改為本機登出並提示；刪除工作區時保證至少保留一個；選單失焦即關閉；未來日期交易需確認；查無代號與查詢失敗分開提示；過期報價降低強調；自選股移除只鎖定該列、復原保留原排序、重複加入顯示「已在自選股中」。
- ⚡ **報價管線**：缺少報價時間時顯示「時間未知」；暖機快取於登出時清空並設 LRU 上限；預抓兩段改為併行；美股中文名搜尋排序比照台股。
- 🛠️ **管理後台與 AI**：角色變更與「全部執行」需確認；狀態頁定時更新並以伺服器時間計算「N 分鐘前」；錯誤區分權限不足／網路錯誤／尚未部署；日誌翻頁游標改為（時間, id），同秒多筆不再重複或遺漏；日誌訊息截斷並遮蔽網址查詢字串；AI 提問失敗保留問題、離開頁面即中止請求、提示詞長度上限。
- 🔒 **維運**：Cloudflare Pages 加上 CSP；PWA 加入 maskable 圖示；Edge 型別檢查不再允許隱含 any；GitHub Actions 釘選到 commit SHA；新增 DEV 排程 e2e（缺憑證時自動略過）。
- ✅ **驗證**：151 個測試檔／2,620 條測試：2,613 通過、7 略過、0 失敗；`npm run build`、`npm run typecheck:edge` exit 0。DEV 與 PROD 皆套用 `admin_recent_app_logs` DDL 並重新部署 `stock-report`、`stock-price`、`backup-transactions`（兩區 bundle sha 相同）。

### 0.9.65（2026-09-23）— 全專案稽核修補第 3 批：Edge 穩定性與維運（Task 166）

- 🔁 **上游抓取全面加上重試與時間預算**：新增共用的 `fetchWithRetry`（逾時、最多 2 次重試、指數退避、429 的 `Retry-After` 上限 5 秒、整體時間預算），六個上游抓取改用；MOPS 回補每輪之間加 300 毫秒間隔。MOPS 單次最壞耗時從 61.6 秒降到 20.4 秒，符合排程的 60 秒上限。
- 🧯 **背景作業的靜默失敗**：探針失敗會記錄失敗種類、Storage 清理改為分頁且讀取失敗即整輪跳過、暖機日期後援會避開沒有資料的日子、總經與匯率同步都有時間預算並回報略過檔數。
- 🔐 **管理與存取**：不得撤銷最後一位管理員（並在事後偵測到零管理員時自動還原），每次角色變更寫入 `app_log`；請求本文 1MB 上限（含 chunked）；錯誤堆疊只保留 3 層並遮蔽長字串；使用者清單分頁；日誌查詢筆數上限 200。
- 📈 **報價服務**：MIS 交易時間無法解析時不再拿買一價當成交價；缺 `Authorization` 直接回 401；超過 50 檔會回報被截斷的代號；Yahoo 後援加 20 秒整體期限；`.TW`／`.TWO` 候選邏輯抽成共用函式；解析錯誤與查無代號分開記錄；TPEx 靜態後援清單會回報快照年齡，超過 90 天會告警。
- 🧪 **資料來源防呆**：融資融券表加欄名檢查、FRED 抓取失敗與「今天沒有新資料」分流、匯率加單日漲跌幅合理性檢查、MOPS 巢狀表格視為解析失敗而非錯誤數字。
- 🗂️ **資料庫維運**：四張紀錄表新增 180 天保留排程、`backup_run_log` 補上外鍵、`config.toml` 宣告四個 Edge Function 的 `verify_jwt`。
- ✅ **驗證**：150 個測試檔／2,605 條測試：2,598 通過、7 略過、0 失敗；`npm run build`、`npm run typecheck:edge` exit 0。DEV 與 PROD 皆套用 DDL 並重新部署 `stock-report`、`stock-price`、`backup-transactions`。

### 0.9.64（2026-09-23）— 全專案稽核修補第 2 批：股利、費用口徑、分割原子化（Task 166）

- 💰 **新增股利交易**：現金股利（每股股利 × 配發股數 − 代扣費用）只計收入，不動股數與成本；股票股利只增加股數、成本僅加相關費用，均價隨之攤薄。年度報表新增「股利」欄、「股利合計」KPI 與「總報酬（已實現＋股利）」，CSV 匯出入可往返，費用重算不會再誤收股利列。
- 🧮 **回本價與未實現損益同口徑**：回本價改用每批交易自己的費率（BUG-079），求不出答案時回傳「—」而不是 0；美股列的券商口徑顯示「不適用」；報酬率只在成本大於 0 時計算。
- ⚙️ **費率設定**：反推費率超出法定上限會明確回報、費用校正改以整數比較、費率寫入雲端失敗會顯示在畫面上、不再讀取殘留的工作區最低手續費（BUG-084）。
- 🔒 **分割與費用重算改為整批原子更新**：改走單一 `apply_transaction_updates` RPC，整批成功或整批不生效，不再有做到一半的帳。新增 `tx_split_log` 紀錄已套用的分割，重複套用同一筆分割會先警告並要求二次確認。
- 📊 **年度報酬率**：超賣的賣出腿不計入分子與分母（DA-07）。
- ✅ **驗證**：150 個測試檔／2,599 條測試：2,592 通過、7 略過、0 失敗；`npm run build`、`npm run typecheck:edge` exit 0。DEV 與 PROD 皆套用 DDL 並重新部署 `stock-report`。

### 0.9.63（2026-09-22）— 全專案稽核修補第 1 批（Task 166）

- 🔐 **AI 分析改為只限管理員**：`ai-proxy` 未登入回 401、非管理員回 403（不讀設定、不打上游）；前端非管理員不顯示 AI 分頁；`get_ai_settings()` 只對管理員回傳 openai-compatible 金鑰（RISK-013 收斂）。
- 🔐 **帳號安全**：密碼最少 8 碼（註冊、重設密碼、變更密碼），Supabase Auth 開啟信箱驗證；DEV 與 PROD 設定皆已同步。
- 📥 **CSV 匯入不再重複**：以（日期、市場、代號、買賣、單價、股數、費稅、性質）多重集合比對既有交易，重複列在預覽中標示且預設不匯入，可勾選「仍要匯入」。
- 🛡️ **CSV 匯出防公式注入**：以 `=`、`+`、`-`、`@` 開頭的文字欄位加單引號，匯入時自動還原；支援民國年日期（民國 60 年以下視為無效，避免與西元 2 位年混淆）；貼上改為 250 毫秒 debounce。
- 🌙 **夜間批次不再靜默失敗**：沒有交易日資料時不再上傳空報告；上傳回傳失敗會計入失敗清單；籌碼逐檔檢查時間預算並回報略過檔數（`batch_run_log.skipped_for_budget`）；manifest 只在「失敗＋略過 ≤ 20%」時才前進；`generate-all` 任一階段失敗都會寫入 `app_log`；後台手動執行頁會顯示略過與 manifest 未更新。
- 🗑️ **移除 Cloudflare R2 異地備份**：兩個專案都沒有 R2 secret，同步永遠是 skipped。刪除 `r2.ts` 與同步呼叫、後台顯示，並移除 `backup_run_log` 的 `r2_status` / `r2_error` 欄位（RISK-011 結案）。
- ✅ **驗證**：150 個測試檔（149 通過、1 略過）／2,581 條測試：2,574 通過、7 略過、0 失敗；`npm run build`、`npm run typecheck:edge` 皆 exit 0。DEV 已套用 DDL、部署 `ai-proxy`／`stock-report`／`backup-transactions`，並確認未授權呼叫回 401、非管理員讀不到金鑰。

### 0.9.62（2026-09-22）— 移除帳號選單的「匯出我的紀錄」

- 🗑️ **移除「匯出我的紀錄」**：右上角帳號選單不再提供個人自助匯出，其他選單項目（Discord 通知設定、變更密碼、登出等）不變。
- 🧹 一併刪除已無人使用的 `selfExport.ts` 與其測試，並移除後台「備份」頁中提及此功能的說明文字。
- ✅ **驗證**：150 個測試檔（149 通過、1 略過）／2,587 條測試：2,580 通過、7 略過，0 失敗；`npm run build`、`npm run lint` 皆 exit 0。

### 0.9.61（2026-09-21）— 經濟快報也能推送真實內容驗證（Task 165 步驟 2h）

- 🧪 **經濟快報新增「預覽快報」與「預覽完整版」兩顆按鈕**：把真實的經濟快報內容送到你自己的 Discord 頻道，用來確認頻道設定正確、也先看看排版。訊息標題會標上「【預覽】」，不會和排程發送的那一份混淆。
- 📅 非交易日（例如週末）也測得動：找不到當天資料時會退回檔案中最近一個交易日。
- 🔓 帳號即使處於「已暫停」狀態仍可預覽 —— 你正是要用它驗證那個開關。訊息只送到你自己的頻道，不經過共用頻道，因此不需要二次確認。
- 🐛 **修正每日手動發送次數上限失效**：「測試經濟快報連線」原本寫入的記錄類型不在配額的計算範圍內，10 次/日的上限從未生效。四顆手動按鈕（兩個測試連線、兩種預覽）現在共用同一個上限。
- ♻️ 後台「預覽快報／預覽完整版」與各帳號預覽改為共用同一段組裝程式碼，行為不變。
- ✅ **驗證**：151 個測試檔（150 通過、1 略過）／2,593 條測試：2,586 通過、7 略過，0 失敗；`npm run build`、`npm run lint`、`npm run typecheck:edge` 皆 exit 0。

### 0.9.60（2026-09-21）— Discord 推播下放各帳號自助管理（Task 165 步驟 2e–2g）

- 🙋 **每個帳號自己設定 Discord 推播**：右上角使用者選單進入「Discord 通知設定」，可各自設定「經濟快報」與「個人持股報告」兩支 Webhook、各自的啟用開關與推播測試。所有帳號（含管理員）看到的是同一個頁面。
- 📡 **經濟快報狀態分兩行顯示**：「全域頻道」說明你是否會在管理員的共用頻道收到，「我的頻道」說明是否另外發一份到你自己的頻道，避免「未設定」被誤解成收不到。
- ⏰ **發送時間由各帳號自己決定**：經濟快報可分別設定快報版與完整版時間，個人持股報告可設定自己的時間，皆為 30 分鐘一格（快報／持股 17:30–20:30，完整版 21:00–23:30）；不設定就沿用預設時間。
- 🏛️ **管理後台只保留全站設定**：全域經濟快報 Webhook 與各帳號的唯讀狀態清單。發送排程設定已移除——時間完全下放各帳號，兩個全域時間（17:30／21:30）改由資料庫端調整。
- 🎨 **Discord 通知設定頁排版修正**：原本網址輸入框與按鈕列之間沒有間距、貼在一起，其餘間距則是 12／14／16px 混用。改為區塊內一律 12px、區塊之間 24px，三個發送時間下拉也對齊同一左緣。
- ⚙️ **排程機制改寫**：原本三個固定的 pg_cron 工作改為一個每 30 分鐘的 `discord-account-tick`，每次醒來只發送該時段到點的帳號；`discord-holdings-daily` 退役。新增兩個唯一索引確保每天每版只送一次。
- 🗂️ **資料庫**：`user_discord_settings` 新增 `market_enabled`、`market_brief_time`、`market_full_time`、`holdings_time` 四個欄位（後三者為 NULL 代表沿用預設）。
- ✅ **驗證**：151 個測試檔（150 通過、1 略過）／2,576 條測試：2,569 通過、7 略過，0 失敗；`npm run build`、`npm run lint`、`npm run typecheck:edge` 皆 exit 0。

### 0.9.59（2026-09-18）— 管理後台選單整合（8 項 → 5 項）

- 🧭 **管理後台左側選單精簡為五項**：帳號、資料更新、AI 設定、Discord、備份。
- 🤖 **AI 設定**：原本的「AI 連線」與「提示詞」合併到同一頁，連線設定在上、提示詞在下，兩塊各自儲存。
- 🔄 **資料更新**：原本的「抓取狀況」「手動更新」「執行記錄」合併為一項，頁內以分頁切換，預設開啟抓取狀況。
- ✏️ 個股 AI 分頁的設定提示與 README 改為指向「管理後台 → AI 設定」。
- 🔒 只調整前端排版，資料表、權限（RLS）與 Edge Function 都沒有變動。
- ✅ **驗證**：
  - 149 個測試檔（148 通過、1 略過）／2,537 條測試：2,530 通過、7 略過，0 失敗。
  - `npm run build`、`npx tsc --noEmit`、`npm run lint` 皆 exit 0；`dev` 上的 GitHub CI 通過。

### 0.9.58（2026-09-18）— Discord 個人持股日報與後台統一管理（Task 165 第二階段）

- 📒 **每個帳號的持股日報**：
  - 平日與快報同一時間（預設 17:30）送到該帳號自己的 Discord Webhook；跨工作區合併，台幣與美元分開，不做匯率換算。
  - 卡片以 Markdown 呈現：一行「未實現合計」與「今日」損益，每檔一行「代號名稱｜張數｜均價｜未實現」；整張顯示「N 張」，有零股顯示「N 股」，美股顯示「N 股」。
  - 報價取最近一個已收盤交易日；空單標「空」，報價較舊的持股標 ⚠️。
  - 持股只會送到該帳號自己的網址，不提供「繼承全域」，不會出現在共用頻道。
- 🛠️ **Discord 設定改由管理員在後台統一管理**：
  - 主控台「Discord」頁依序為：全域 Webhook、各帳號設定、發送排程、說明；最近發送紀錄預設收合。
  - 各帳號設定：左側帳號清單點一下即切換，右側標示目前帳號與上次發送。
  - 經濟快報可選「繼承全域」（不另外發送）或自訂網址（同一時間多送一份到該帳號頻道）。
  - 個人持股報告可設定網址、開關每日推送、測試連線、「完整推送測試」（實際送出持股卡片，回報資料日與無報價檔數）、清除網址；手動發送每個帳號每天最多 10 次。
  - 使用者選單原本的「Discord 推播」對話框移除。
- ⏰ **發送時間可在後台調整**：每半小時一個下拉選單，快報＋個人持股報告 17:30–20:30、經濟快報 21:00–23:30，預設 17:30／21:30；儲存時直接調整既有排程，不新增排程工作。
- 📰 **經濟快報改用 Markdown**：每列改為「標籤 **數值** 紅綠圓點」，手機上不再因等寬表格折行而擠在一起；國際指數、融資融券、美國總經的寫法同步調整。
- 🐛 **修正持股報價在伺服器上全部抓不到**：持股報價請求原本沒有帶瀏覽器標頭，在 Edge 上一律被 Yahoo 拒絕，卡片顯示 `--`；補上後實測無報價由 7 檔降為 0。
- 🔒 **安全**：
  - Webhook 網址仍只存在伺服器，畫面只顯示末 4 碼，回應與日誌都不含網址；來自資料的文字（股名、指數名）會跳脫 Markdown 符號。
  - 新的後台操作只接受管理員；調整排程的資料庫函式只開放給 service role，並在資料庫端再次檢查時間範圍。
- 🗄️ **資料庫與排程**（`schema.sql` §14、§15）：
  - 新增 `user_discord_settings`（含每帳號經濟快報網址 `market_webhook_url`）、`user_discord_send_log`、排程 `discord-holdings-daily`（`30 9 * * 1-5`），以及 `discord_schedule_get`／`discord_schedule_set`。
  - §13 快報預設時間改為 17:30（`30 9 * * 1-5`）。
  - `verify.sql` 期望表數 17 → 19，並檢查兩張新表的 RLS。
  - 部署時需另外套用 §14、§15，以複製既有排程指令的方式建立持股排程，並以 `--no-verify-jwt` 重新部署 `stock-report`。
- ✅ **驗證**：
  - 149 個測試檔（148 通過、1 略過）／2,537 條測試：2,530 通過、7 略過，0 失敗。
  - `npm run build`、`npx tsc --noEmit`、`npm run typecheck:edge`、`npm run lint`、`sync-edge-engine --check` 皆 exit 0。
  - DEV 實際部署：以臨時管理員帳號跑過後台所有操作（含非管理員 403），`verify_setup()` 10 項全數 PASS；兩張卡片以真實資料送達測試頻道。

### 0.9.57（2026-09-17）— Discord 每日盤後總結（Task 165 第一階段）

- 📨 **每日盤後總結推送到 Discord**：
  - 平日 17:05 送「快報」、21:30 送「完整版」到管理員設定的單一 Discord Webhook，兩則皆為一般推播。
  - 快報含台股大盤、三大法人、國際指數、匯率；完整版再加上融資融券與美國總經。
  - 當天 `market/daily.json` 沒有台股大盤資料（含國定假日）就不送，只記在發送紀錄。
  - 同一天同一版只送一次（`discord_send_log` 部分唯一索引），排程重跑不會重複發送。
  - 第一階段只含全市場資料，不讀取、不送出任何使用者的持股或交易。
- 🧾 **版面：分區卡片＋等寬表格**：
  - 每個區塊一張卡片，卡片內以等寬程式碼區塊排成表格（依顯示寬度補空白），行尾以 🔴 漲／🟢 跌／⚪ 平標示方向；台股大盤卡依漲跌、三大法人卡依合計買賣超上色。
  - 每張卡片底部標資料時間（例如「09/17 15:05 更新」）；台股大盤、三大法人、融資融券、匯率若非當日資料，照放數字並標「⚠️ 09/16 資料・非今日」。
  - 三大法人「盤後初步／完整」依盤後資料的更新時刻判斷（台北 19:30 為界）。
  - 融資融券合計（證交所 `MI_MARGN` 信用交易統計）與 8 檔國際指數（Yahoo，只取已收盤的交易日）於發送當下抓取，不寫入既有報表檔、不改動既有抓取批次。
- 🛠️ **管理員主控台新增「Discord」頁**：
  - 設定、清除 Webhook 網址；「測試發送」確認連線。
  - 「預覽快報」「預覽完整版」以當下真實資料立即送出正式內容（標題加【預覽】），不佔用當日排程名額；當天沒有資料時改用最近一個交易日並標示資料日期。
  - 可展開的「如何取得 Discord Webhook 網址」步驟說明（不需要建立 Bot，也不需要 API 金鑰）。
  - 最近發送紀錄顯示台北時間（`YYYY-MM-DD HH:mm:ss`）、類型、狀態、HTTP 狀態碼與原因。
- 🔒 **Webhook 網址視同密碼**：
  - 存在只有伺服器（service role）讀得到的 `app_secrets` 表：RLS 開啟且沒有任何 policy，`anon`／`authenticated` 無任何權限。
  - 主控台只顯示網址末 4 碼；回應、錯誤訊息與日誌都不含網址；只接受 `discord.com`／`discordapp.com` 的 Webhook 網址。
  - 快照一律排除 `app_secrets`（含 `--with-cache`），網址不會進入異地備份。
  - 所有 Discord 訊息都關閉 @mention 解析。
- 🗄️ **資料庫與排程**（`schema.sql` §13）：新增 `app_secrets`、`discord_send_log`，以及 `discord-summary-brief`（`5 9 * * 1-5`）、`discord-summary-full`（`30 13 * * 1-5`）兩個排程；`verify.sql` 期望表數 15 → 17，並檢查兩表的 RLS。部署時需另外套用 §13，並以 `--no-verify-jwt` 重新部署 `stock-report`。
- ✅ **驗證**：
  - 137 檔測試檔 / 2,208 條測試：2,201 通過、7 略過，0 失敗；版面與時間相關測試另以 `TZ=UTC` 重跑通過。
  - `npm run build`、`npm run typecheck:edge`、`npm run lint` 皆 exit 0。
...
- 👀 **手機版觀察股票標題列排成一行**：「觀察股票」不再斷成兩行；「圖卡／條列」只顯示圖示（保留「圖卡模式／條列模式」的無障礙名稱），按鈕 44×44；「加入觀察」縮短為「＋ 加入」（無障礙名稱仍為「加入觀察」）。
- 🖥️ **桌機版完全不變**：以 Playwright 在 1440 px 比對修改前後，工具列四顆按鈕、搜尋欄、觀察股票標題與按鈕的位置與尺寸全部相同。
- ✅ **測試**：新增 2 條（`TransactionsPage.tools`：「工具」選單列出四項、點選後關閉選單並打開對應功能，先紅後綠）；1866 條全部通過；`npm run build`、`npm run typecheck:edge` 皆 exit 0。
- ℹ️ **不需部署 Edge Function**：純前端改動，仍需手動上傳 Cloudflare Pages。

### 0.9.47（2026-09-11）— 桌機版「新增交易」改回右下角浮動按鈕

- 🔁 **桌機版改回右下角浮動按鈕**（使用者決定，撤回 Task 159 D9）：0.9.45 移到頁首的「新增交易」改回右下角、有圖示也有文字的浮動按鈕，和手機版的位置一致。頁首按鈕移除。
- 📏 **桌機版頁面底部原本就有 96 px 留白**：寬度 > 720 px 時，`main.container` 本來就預留 96 px，捲到最底時最後一列會露出來（實測距按鈕 109 px），因此本版不需另加規則。捲動途中，按鈕仍可能暫時蓋到某一列最右側的欄位。
- 📱 **手機版不變**：維持 56×56 的圖示按鈕；管理後台仍然不顯示。
- ✅ **測試**：更新 1 條（`AppShell.a11y`：桌機也必須是浮動按鈕、不得出現頁首按鈕，先紅後綠）；1864 條全部通過；`npm run build`、`npm run typecheck:edge` 皆 exit 0。另以 Playwright 在 1440、1024、375 三種寬度確認按鈕位置與底部留白。
- ℹ️ **不需部署 Edge Function**：純前端改動，仍需手動上傳 Cloudflare Pages。

### 0.9.46（2026-09-11）— 修正 iPhone 實機上的版面問題

- 🐛 **修正 iPhone 下拉選單的字被切掉**（BUG-080，0.9.45 造成）：0.9.45 把手機的 16 px 字級與 12 px 上下內距規則也套用到 `select`，但高度仍是 40 px，iOS 只剩 16 px 給文字，「台股」「買入」「現股」「張」的上半部被切掉。寬度 ≤ 720 px 時，輸入框與下拉選單改為高 44 px、上下內距 0；多行文字框維持 12 px 內距。
- 📅 **修正 iOS 日期欄位過高、日期置中並超出欄位**：`input[type="date"]` 取消原生外觀並設 `min-width: 0`，日期靠左對齊。
- 📈 **修正個股走勢圖標題被擠成直排**：標題列允許換行，「走勢圖」與日期標籤不再斷行；寬度 ≤ 560 px 時，八個區間按鈕排成 4×2 等寬、高 40 px。
- 🛠️ **修正管理後台「重新整理」被擠成直排**：該列允許換行，按鈕文字不斷行。
- 🙈 **管理後台不再顯示「新增交易」按鈕**：它會蓋住狀態文字，而且在後台沒有用途。
- ✅ **驗證**：1864 條測試全部通過；`npm run build`、`npm run typecheck:edge` 皆 exit 0。以 375 px 測試頁比對修正前後：「走勢圖」標題 14×60 → 42×20（一行），日期標籤 50×75 → 132×23，區間按鈕兩列，輸入框與下拉選單 44 px 高。iOS 專屬的下拉選單與日期欄位要在實機上確認。
- ℹ️ **不需部署 Edge Function**：純前端改動，仍需手動上傳 Cloudflare Pages。

### 0.9.45（2026-09-11）— 手機與桌機介面第一批改善

- 📱 **手機「新增交易」不再大片遮住內容**（Task 158 #2）：寬度 ≤ 720 px 時改成 56×56 的圖示按鈕，頁面底部預留導覽列加按鈕的空間，最後一列可以捲到按鈕上方。
- 🖥️ **桌機「新增交易」移到頁首**（Task 159 D9）：不再浮在右下角蓋住「未實現報酬率」欄；全站只會有一個「新增交易」按鈕。
- ⌨️ **iPhone 點輸入框不再自動放大，數字欄叫出數字鍵盤**（Task 158 #5、#6）：寬度 ≤ 720 px 時所有輸入框為 16 px；單價、股數、手續費率、最低手續費加上 `inputmode="decimal"`；手機上股數欄獨佔一行，不再被「張／股」選單擠成 50 px。
- 🧾 **交易表單逐欄指出錯誤**（Task 159 D6）：代號、單價、股數各自顯示錯誤並標示 `aria-invalid`，焦點移到第一個錯誤欄位；改正後該欄的錯誤立即消失。
- 💡 **手續費率顯示成折數，填錯會警告**（Task 159 D2）：台股費率下方顯示「＝ 6 折」；高於原價 0.001425 時提醒「是不是把折數填成小數」。只是提示，儲存的數值與計算方式都沒有改變。工作區的「預設手續費率」對話框同樣適用。
- 🔔 **錯誤通知不再 5 秒自動消失**（Task 159 D4）：錯誤訊息保留到使用者按關閉；成功訊息維持 5 秒。
- 🌗 **深色主題對比達到 WCAG AA**（Task 159 D8）：紅色損益 `#fa4d56` → `#ff8389`（斑馬紋列上 3.77 → 5.33:1）；次要灰字 `#8d8d8d` → `#a8a8a8`（3.81 → 5.31:1）；表頭「?」改用次要文字色。
- 🆕 **新帳戶首頁不再是一堆 0**（Task 159 D11）：沒有持股時隱藏兩張摘要卡，空狀態提供「新增第一筆交易」與「匯入 CSV」兩個按鈕。
- ⏳ **報價取不到時不再一直顯示載入動畫**（Task 158 #16）：載入完成後仍沒有報價的欄位顯示「—」，摘要卡顯示「目前取不到美股報價」與「重試」。載入中仍顯示骨架。
- ♿ **頁面結構與分頁標題**（Task 159 D12、D13，以及 D1 的標題部分）：每頁有一個給螢幕閱讀器的 `<h1>`；分頁標題改為「交易紀錄 · 股票小幫手」的格式；首頁標題層級改為 h1 → h2 → h3；「Active 持股」改為「目前持股」。
- 📲 **加到主畫面有正確的圖示**（Task 158 #15）：新增 180／192／512 px 的 PNG 圖示與 `apple-touch-icon`。
- 🧱 **`100dvh`**（Task 158 #9）：`body`、`#root` 在 iOS Safari 不再比實際畫面高。
- ✅ **測試**：新增 19 條（`feeRateHint`、`Toast`、`DashboardPage`、`AppShell.a11y`、`TransactionForm.validation`，先紅後綠），並更新 1 條過時的煙霧測試（空帳戶不再顯示摘要卡，改成先寫入持股）。總數 1845 → 1864，全部通過；`npm run build`、`npm run typecheck:edge` 皆 exit 0。另以 Playwright 在 iPhone 13 mini（375×629）與 1440×900 重新量測確認。
- 📄 **稽核報告**：`docs/design/mobile-ux-audit-iphone13mini.html`、`docs/design/desktop-ux-audit.html`。其餘項目列在 Task 158／159 的第 2、3 批。
- ℹ️ **不需部署 Edge Function**：純前端改動。前端仍需依 README 步驟 9-1 手動上傳才會上線。

### 0.9.44（2026-09-10）— 修復查無檔案被當成錯誤，即時產生因此永遠不會執行

- 🐛 **修復非批次股票的基本面分頁永遠空白**（BUG-078）：`downloadReportsJson` 只把 **404** 當成「檔案不存在」，但 Supabase Storage 對不存在的物件回的是 **HTTP 400**（實測 PROD，body 為 `{"statusCode":"404","code":"NoSuchKey"}`）。於是「還沒產生」的檔案會走進 `throw`。
- 🔑 **真正的傷害是那個 throw，不是錯誤紀錄**：每個呼叫端都是「先讀檔，讀不到才呼叫 `warmStockCore` 即時產生」，兩行在同一個 `try` 裡。拋錯跳過即時產生 → 檔案永遠不會被建立 → 下次再訪重複同樣的拋錯。`2382` 因此從 0.9.38 起一直是空的。
- 🎯 **一處修好七個 proxy**：籌碼、日線、基本面、大盤、外資、總經、匯率全部共用這個函式，過去一律把「還沒產生」當成失敗。
- 🛡️ **真錯誤仍然可見**：body 不是查無物件的 400、以及 500、網路中斷、壞 JSON，全部照舊拋錯。`null` 只代表「不存在」。
- ✅ **測試**：新增 4 條（`reportsBucket.test.ts`，先紅後綠），總數 1841 → 1845，全部通過；三道 gate 皆 exit 0 且無 unhandled error。
- ℹ️ **不需部署 Edge Function**：純前端改動。0.9.43 的 Edge 5 年後援保留，它涵蓋「即時產生本身失敗」的情況；本版修的是讓即時產生**有機會被執行**。

### 0.9.43（2026-09-10）— 修復非持股股票的日線區間讀不到

- 🐛 **修復行情分頁四個日線區間對非持股股票全部失敗**（BUG-077）：`近 1 月`、`近 6 月`、`本年迄今`、`近 1 年` 讀的是 `daily/{ticker}.json`，而那份檔案只有持股才有。實測 PROD 的 `daily/` 只有 9 個檔案，連 `2330` 都沒有。0.9.42 讓行情分頁多了四個讀這份檔案的區間，於是任何非持股股票一開就有四個區間壞掉。`一日`／`五日` 與 `近 5 年`／`全部` 因為都走 Edge 到 Yahoo，不受影響。
- 🔁 **`useDailySeries` 加上第三層後援**：檔案讀不到、`warmStockCore` 也產不出來時，改打 0.9.41 就已上線的 Edge `daily` action 取五年日線再合成序列。修在 hook 這一層，行情與技術面兩個分頁一起修好，兩個元件都不必改。
- 💡 **後援刻意取 `5y` 超集**：一次請求供應四個本地區間，之後點 `近 5 年` 也免費，因為 `fetchRemoteDaily` 依 ticker 與 range 快取。**不需要改 Edge Function，也不需要重新部署。**
- 🧪 **修掉四個測試檔的完整替換式 mock**：它們用 `vi.mock` 整包替換 `dailyProxy` 而沒有 `fetchRemoteDaily`，本次改動後會產生 32 個 unhandled error。這些錯誤**不會讓摘要行變紅**——`vitest` 回 exit 1，摘要仍寫「1841 passed」。判定依據永遠是 exit code 與 `Errors` 那一行。
- ✅ **測試**：新增 5 條測試（`useDailySeries.test.ts`），總數 1836 → 1841，全部通過；`npm run build`、`npm run typecheck:edge`、`npx vitest run` 三道 gate 皆 exit 0 且無 unhandled error。

### 0.9.42（2026-09-10）— 行情走勢圖也擴充為八個區間

- 📈 **行情分頁的走勢圖改為八個區間**：`一日`、`五日`、`近 1 月`、`近 6 月`、`本年迄今`、`近 1 年`、`近 5 年`、`全部`。八個區間都是折線；K 棒與指標仍然只在技術面分頁。
- ↩️ **這一版反轉了 0.9.41 的決定**：0.9.41 詢問時的選擇是「只擴充 K 線圖的選擇器」，使用者看到成果後改為兩張圖都要。`chart-range-eight.md` §7 的「`IntradayChart.tsx` 不需改動」已加註修訂。
- ♻️ **`IntradayChart` 的 `series` 型別縮小為 `TrendSeries`**：該元件實際只讀 `points`、`prevClose`、`symbol`，所以日線組出來的序列不必假造一個 `interval`。
- ♻️ **props 改為泛型 `<R extends TrendRange>`**：`TwIndexToday.tsx` 因此完全不用改。直接把 `range` 放寬成 `TrendRange` 會讓它的 `onRangeChange={setRange}` 因參數逆變而編譯失敗。
- 🧮 **日線區間不顯示均價**：一年的累積 VWAP 沒有意義，均價線與 `均價` 欄位在日線區間隱藏，盤中區間的數字完全未動。
- 🐛 **修復日線讀取失敗時轉圈停不下來**：effect 讀 `dailyStatus` 卻沒把它放進相依陣列，而 `useDailySeries` 失敗時只改狀態、不動 `series`，所以 effect 不會重跑。改為精準相依：只有本地日線分支消費該狀態，其餘分支凍結成常數，避免預設的一日區間在日線背景載入完成時無謂重抓並閃爍骨架。
- ✅ **測試**：新增 22 條測試，總數 1814 → 1836，全部通過；`npm run build`、`npm run typecheck:edge`、`npx vitest run` 三道 gate 皆 exit 0。
- ℹ️ **本版不需部署 Edge Function**：純前端改動。`近 5 年` 與 `全部` 仍依賴 0.9.41 的 `daily` action，DEV 已上線（v7），**PROD 尚未部署**。

### 0.9.41（2026-09-10）— 個股走勢圖區間擴充為八個

- 📈 **技術面日 K 圖的區間選擇器改為六個**：`近 1 月`、`近 6 月`、`本年迄今`、`近 1 年`、`近 5 年`、`全部`。盤中走勢圖的 `一日` / `五日` 維持不動，兩者合計為八個區間。原本的 `近 3 月` 按鈕移除。
- 🌐 **`近 5 年` 與 `全部` 不落地 Storage，改由 Edge Function 即時代理**：`stock-price` 新增 `daily` action，接受 `range: '5y' | 'max'`。夜間批次與 `daily/{ticker}.json` 完全未動，Supabase 的儲存量、批次時間與 cron 皆不變。
- 📉 **`全部` 是月線，不是日線**：2026-09-10 實測 `2330.TW`，`range=max` 帶 `interval=1d` 時 Yahoo 仍回月線（321 根、28 KB）；要真正的全歷史日線必須改用 `period1=0`，那是 6643 根、566 KB，對單次請求與 SVG 繪製都過重，故不採用。
- 🐛 **月線時間戳必須先加 `meta.gmtoffset` 才能取日期**：不加會讀成上個月月底（實測 `2000-02-01` 變成 `2000-01-31`），整條月線位移一格。
- 🐛 **丟棄 `range=max` 附加的即時列**：該列的 timestamp 等於 `meta.regularMarketTime`，不丟的話當月會出現兩根 K 棒。
- 🐛 **丟棄 OHLCV 全為 null 的空格**：實測 `range=5y` 的 1215 根裡有一根（2025-08-01）五個欄位皆為 null，保留會讓均線斷線。
- 🐛 **修復切換遠端區間時沿用前一個區間的資料**：`近 5 年` 切到 `全部` 時，舊資料會掛在新標籤下畫出來，而且不出現載入指示。原因是清除舊資料只寫在「切回本地區間」那個分支裡。此缺陷由 reviewer 找出，已補上會抓到它的回歸測試。
- 🐛 **遠端讀取失敗不再快取**：原本失敗結果會被快取 5 分鐘，網路瞬斷後使用者連點五分鐘都只會重播那個失敗，不會再試一次。現在只快取成功結果。
- ♻️ **`本年迄今` 的年份取自最後一列，不看系統時鐘**：讓區間計算維持純函式，也不必處理時區。
- ✅ **測試**：新增 21 條測試，總數 1793 → 1814，全部通過；`npm run build`、`npm run typecheck:edge`、`npx vitest run` 三道 gate 皆 exit 0。
- ⚠️ **本版需部署 Edge Function 才會生效**：`supabase functions deploy stock-price`。未部署時 `近 5 年` 與 `全部` 會顯示「讀取長區間股價失敗，請稍後再試。」

### 0.9.40（2026-09-10）— Edge Function 台股清單補上完整性檢查

- 🐛 **修復 Edge Function 回傳截斷的台股清單卻標示成功**（BUG-076，結案 RISK-008）：`stock-price` 的 `twlist` action 過去只要上市或上櫃其中一個來源成功就合併回傳，另一個掛掉時呼叫端會拿到帶 HTTP 200 的半份清單並快取 30 分鐘，期間該交易所的每一檔股票都顯示為「查無此股」。0.9.39 已修客戶端直連路徑，本版補上 Edge 端。
- 🧪 **完整性判斷改為逐來源計算有效列數**：只看陣列長度擋不住「非空但每一列都缺代號或缺名稱」的來源。有效列數刻意在去重之前計算——僅與另一交易所重複的列仍證明該來源有回應。
- ♻️ **純邏輯抽離便於測試**：新增 `sources/supabase/functions/stock-price/twList.ts`，不含 Supabase／Deno 匯入，比照 `backupPlan.ts` 的既有模式，新增 8 條單元測試。
- ⚠️ **本版需部署 Edge Function 才會生效**：`supabase functions deploy stock-price`。程式碼合併不等於上線。

### 0.9.39（2026-09-10）— 股票搜尋載入指示、觀察清單兩個缺陷修復與字卡版面重整

- ⏳ **買入股票的名稱搜尋補上載入指示**：交易表單的股票名稱欄位會在去抖動 300ms 後打遠端搜尋，過去全程沒有任何回饋，使用者無法分辨「搜尋很慢」與「根本沒在搜尋」。現在第一個按鍵就顯示轉圈（不等去抖動），搜尋期間只顯示轉圈列，不會留著上一個關鍵字的過期結果。
- ⏳ **加入觀察對話框補上清單載入指示**：關鍵字過濾本身是同步的本地比對，不需要轉圈；真正的空窗是開啟時抓取台股清單。清單未到位時顯示「載入台股清單…」，成功與失敗都會停止轉圈。
- 🎨 **新增 Carbon Design 載入指示器元件**：新增 `sources/src/components/Common/Spinner.tsx`，灰色軌道圓加 3/4 藍色弧線，690ms 一圈，取色自 `--cds-layer-accent-01` 與 `--cds-interactive`，並在 `prefers-reduced-motion: reduce` 下停止動畫。既有 `.spin` 與其 18 個使用處完全未動。
- 🐛 **修正切換交易性質會留下過期搜尋結果**：`nature` 是 `isSpotSell` 的一部分，切換它會隱藏搜尋下拉，但過去不會清掉搜尋狀態；切回來會看到上一個關鍵字的結果。此為既有缺陷，本次一併修掉。
- 🐛 **修正表單關閉後仍會發出搜尋**：去抖動計時器沒有 unmount 清理，表單在 300ms 內關閉時搜尋仍會照常發出。已補 cleanup。
- 🐛 **修復加入觀察搜尋把已加入的股票靜默隱藏**（BUG-074）：搜尋結果過去會把已在觀察清單中的代號整筆移除且不給任何說明，使用者輸入代號後看到空白清單，得到的訊息是「這檔股票不存在」，事實是「已經加過了」。現在改為列出但停用、標示「已在觀察清單」，並排到可加入項目之後。
- 🐛 **修復部分來源失敗的殘缺台股清單被當成完整清單快取**（BUG-075）：上市（TWSE）與上櫃（TPEx）兩個來源過去只要有一個成功就合併回傳，另一個掛掉時會得到半份清單並被快取 30 分鐘，期間該來源的每一檔股票都顯示為「查無此股」。現在任一來源失敗即整體視為失敗，交給既有的後援鏈處理。回 HTTP 200 但內容為空陣列的情況一併視為失敗。
- 🔑 **台股清單快取換版**：`tw-list-v1` 改為 `tw-list-v2`，避免沿用修正前寫入的殘缺快取。使用者升版後第一次開啟會重新抓取清單。
- 🎨 **重整自選股字卡的名稱與產業別版面**：代號、名稱、產業別過去擠在同一行且字級相同，在 136px 的欄寬下名稱與產業別會同時被截斷成省略號，糊成一團。現在產業別若與上方群組標題相同就不重複顯示；其餘情況移出名稱列、獨立成行、去掉外框與底色改用輔助文字色，名稱因此獨佔整行寬度。
- ✅ **測試**：新增 22 條測試，總數 1760 → 1782，全部通過；`npm run build` exit 0。其中四條回歸測試已反向驗證，移除對應修正後確認轉紅。
- ⚠️ **已知風險**：`fetchViaEdge` 仍無完整性檢查（RISK-008，需改 Edge 端）；「還有 N 筆」計數含已加入、不可點的項目（RISK-009）。兩者記錄於 `BUG_FIX.md`，本版不修。
...
- 📐 **高質感向量設計與無縫融入**：採用實心正圓頭部與雙層俐落流線肩弧（線寬 2.2px / 1.8px），完美居中於 30px 圓徽中；全面支援深色與淺色主題下的高對比度前景色（`var(--steel-on)`），帶來通透輕盈的現代 Fintech 視覺質感。
- 📚 **完整設計提案與互動展示文件**：於 `docs/avatar-icon-designs.html` 與 `docs/design/` 提供 6 款人像主題現代扁平設計提案、實機 Header 頂欄動態模擬及視覺設計總覽圖板。

### 0.9.35（2026-09-06）— 全域錯誤記錄 `app_log`：捕捉層、後台檢視頁與全站埋點

> 本專案先前無法呈現執行期發生了什麼事。Edge Function 完全沒有 `console.log`，例外只變成一個
> 沒人保存的 HTTP 回應；前端沒有 ErrorBoundary，service 一律 catch 後 `return null`，失敗渲染
> 成空狀態；既有的四張 log 表只記錄排程的**結果**，不記錄原因。使用者回報「畫面一片空白」時，
> 系統裡沒有留下任何紀錄。本版補上缺少的捕捉層，並在後台加上檢視頁。
>
> DEV 實機驗證：真實 Edge 例外落表（含堆疊，`detail` 無多餘欄位）、`app-logs` 對無憑證與
> 僅帶 anon key 皆回 401、body 為 `null` 回 400、RLS 五案行為符合設計、App 啟動無任何
> console／page 錯誤。

#### Phase 1 — 資料表、Edge 捕捉與後台面板
...
#### Phase 2／3 — ErrorBoundary、14 個靜默 catch 與帳務寫入埋點
...
- 🐛 **收盤後報價定案判定（BUG-050）**（`quoteWindow.ts`、`priceProxy.ts`、`holdingRows.ts`、`QuoteTab.tsx`、`StockDetailPage.tsx`、`stock-price/index.ts`）—
  - 新增 `twIsAfterClose(at)`：判斷某個台北時刻是否落在「當天不可能再產生新價格」的區間（14:00 沉澱窗結束之後，或 08:25 開盤前）。
  - `twQuoteTtlMs()` 新增第三個參數 `fetchedAt`。撮合時間無法證明已定案時，改以「這一列是什麼時候抓的」判斷：收盤後才抓到的列鎖到隔天 08:25，取代原本整晚每十分鐘重抓。
  - **刻意不變的部分**：省略 `fetchedAt`、或 `fetchedAt` 仍在盤中時，一律維持 0.6.42 的十分鐘退避。盤中抓到的列可能只是盤中快照，鎖定它正是 BUG-011 的原始缺陷。
  - `isClosed(quote, market)` 新增市場參數，只對台股套用台北時鐘推論；美股時段正好落在台北 14:00 之後，若不分市場就會誤判。未帶市場參數的呼叫端行為完全不變。
- 🐛 **反向分割算出 0 股時擋下確認（AUDIT-09）**（`StockSplitModal.tsx`）— 3 股的零股做 10 併 1 時 `Math.round(3/10)` 為 0，但原手續費原封不動帶過，`pnlEngine` 於是把手續費加進 `pos.cost` 卻沒有對應的 `pos.qty`，該標的的平均成本與保本賣出價被永久墊高。新增警告、確認按鈕停用、`handleConfirm` 提前返回三道關卡。
- 🐛 **批次更新中途失敗時回報進度（AUDIT-11）**（`StockSplitModal.tsx`、`RecalcFeesModal.tsx`）— 逐筆 `await` 的迴圈失敗時只說「更新失敗」，使用者無從得知哪幾筆已經改掉。改為明確回報「已完成 N 筆，第 N+1 筆更新失敗：…。其餘 M 筆未變更」。沒有交易 API 可回滾，誠實告知範圍就是本次的全部需求。
- 🐛 **單一標的失敗不再中斷整輪籌碼產生（AUDIT-12）**（`stock-report/index.ts`）— 逐檔 `try/catch`，失敗計入 `failed` 並把股票代號（最多 10 檔）放進回應主體；`summariseFollowUp` 與管理台的執行摘要都會顯示。
- 🔒 **`CRON_SECRET` 改用固定時間比較（AUDIT-13）**（新增 `stock-report/cronSecret.ts`）— 原本的 `!==` 在第一個相異位元組就返回。`CRON_SECRET` 未設定時仍然一律 401。
- 🐛 **走勢圖漲跌百分比防除以 0（AUDIT-14）**（`IntradayChart.tsx`）— 昨收為 0 時 tooltip 會印出 `(Infinity%)`。抽出 `changeLabel()`，昨收為 null 或 0 時不輸出漲跌標籤，與 `DashboardPage`、`QuoteTab`、`WatchSection` 既有的判斷一致。
- 🐛 **交易日期正規式錨定結尾（AUDIT-15）**（`csv.ts`）— `2024/01/105` 原本被讀成 `2024-01-10` 且不報錯。改為錨定結尾並保留「日期後接時間」的匯出格式。
- ⚖️ **AUDIT-10 判定為可接受風險，不改程式** — 「2.500」在台美股都可能是合法的 2.5 元，加規則拒絕它會擋掉正常匯入；多組點號如「1.234.567」目前已是 NaN 會報錯。台灣券商匯出使用逗號千分位，不會觸發。
- ✅ **測試**：新增 24 個測試（含新檔 `RecalcFeesModal.test.tsx`、`cronSecret.test.ts`），改寫 2 個被本版取代的既有案例；全套 99 檔 / 1,637 項通過，`npm run build` 綠燈。
- 🔍 **Review**：`route:reviewer` 判定 PASS，三項 RISK。其中兩項（失敗診斷資訊、管理台未顯示 `failed`）已於本版一併修正；一項列為可接受風險（見 `BUG_FIX.md`）。

### 0.9.31（2026-09-04）— 修正同日買賣排序不定造成的假性超賣與已實現損益虛增

> 匯入批次寫入的交易共用同一個 `created_at`，`tx_date` + `created_at` 兩個排序鍵同時相等時沒有決勝鍵。PostgreSQL 排序不穩定，可能把 SELL 排在 BUY 之前；JavaScript 的 `Array.sort` 是穩定排序，原封不動保留這個錯誤順序，於是引擎在買進尚未入帳時就處理賣出，誤判超賣並以成本 0 計算。

- 🐛 **同日同 `created_at` 的排序決勝**（`pnlEngine.ts`）—
  - 新增 `compareTxOrder()`：在 `tx_date` 與 `created_at` 都相等時，開倉腿排在平倉腿之前（現股 BUY、融券 SELL 為開倉），最後以 `id` 收尾，結果完全可重現。
  - `computeLedger()` 改用 `compareTxOrder()`；日期或建立時間不同時的相對順序完全不變。
- 🐛 **列表與匯出改用引擎排序**（`TransactionsPage.tsx`）— 交易列表的日期排序與 CSV 匯出改用 `compareTxOrder()`，畫面順序不再與引擎的計算順序不一致。
- 🔒 **兩個資料層都對齊引擎排序**（`dataProvider.ts`）— Supabase `listTransactions` 追加 `.order('id')`，讓 PostgREST 回傳的順序本身就穩定；本機（localStorage）模式的 `listTransactions` 改用 `compareTxOrder()`，兩種模式不會再對相同的平手情境給出不同順序。
- 📊 **實測影響**（PROD 111 筆交易）— 修正前該次載入的全站已實現損益為 878,583，修正後為 299,807，虛增 578,776 元；3037 由 250 股／成本 223,316／已實現 45,548 修正為 200 股／成本 181,257／已實現 3,489。15 組同日買賣配對中有 14 組暴露於同一個排序決勝缺陷。
- ✅ **測試**：新增 6 個測試（3 個引擎排序、3 個 `compareTxOrder`）；全套 1609 通過，`npm run build` 綠燈。

### 0.9.30（2026-09-04）— 庫存總覽持股體驗全面升級、券商牌告口徑直顯與個股產業清晰度優化

> 這一版跨六次迭代（0.9.30-dev.1 ~ dev.6），主線為**庫存總覽持股體驗全面升級、券商月退牌告口徑雙行直顯，以及個股分析產業別清晰度重塑**：庫存總覽直顯券商未實現損益與報酬率消除佣金退折資訊差，分組列章節通欄化並確立 13px 黃金比例徹底消除橫向捲軸；個股分析重塑產業別徽章徹底解決過小、截斷與低對比模糊問題；觀察清單迷你緊湊化。

- 🏷️ **個股分析股票產業別顯示字級與高對比清晰度重塑**（`QuoteTab.tsx`、`index.css`）—
  - 核心痛點：個股分析行情卡片中的產業別徽章原本複合使用 `.watchlist-card-badge`，受到 CSS 定義順序與相同特異性影響，被覆蓋為緊湊小卡的 `max-width: 48px`、`font-size: 9px` 與暗淡低對比色彩 `var(--ink-muted)`（淺色主題下對比度僅 2.8:1）。導致 8 個字長產業名稱（如「電腦及週邊設備業」）被截斷為省略號，繁體中文字筆畫在 9px 下模糊難辨。
  - 行情卡片產業徽章重塑（`.quote-badge`）：
    - 移除 `.watchlist-card-badge` 耦合，確立獨立徽章規格。
    - 字級提升至 **12.5px**（與旁邊 13px 的代碼 `.code` 形成平衡且易讀的比例，各複雜繁體字筆劃清晰展現）。
    - 移除 `max-width: 48px` 限制，長產業名完整平整呈現。
    - 徹底根絕模糊感：文字色彩改為最高對比度的 `var(--ink)`（深色主題 `#dfe4ea` 對比 >12:1，淺色主題 `#0f1319` 對比 >16:1），邊框升級為 `var(--border-strong)`，並加入微字距（`letter-spacing: 0.02em`）與平滑字型渲染。
  - 個股分析標題列與下拉選單同步強化：
    - `.detail-title .badge` 字級提升至 **12px**，色彩改為 `var(--ink)`，維持全頁面和諧一致。
    - 切換選單 `.hmenu-head` 產業分組標題（如「觀察 ── 半導體業」）由淡灰提升為 `var(--ink-secondary)` 並加粗為 `font-weight: 600`。

- 📐 **庫存總覽持股表格黃金比例字級與消除橫向捲軸（零下方滑動）**（`index.css`）—
  - 徹底消除橫向滾動：主文字與數值單元格 `.data-table td`、`.data-table .num` 微調至 **13px** 視覺甜蜜點；表頭 `th` 定位為 **12px**；次級輔助文字維持 **11px**；內距收斂為 `9px 7px`，首末欄左右縮減至 `10px`，釋放近 40px 欄位寬度，在 1,140px 容器下全 10 欄一眼綜覽無須向右滑動。
  - 響應式自適應縮放：新增 `@media (max-width: 1120px)` 智慧微縮（主字 12px、次字 10px、padding 8px 5px），半螢幕多工零滾動條。
  - 設計範本同步更新：`docs/design/holdings-table-mockup.html` 提供 13px 黃金款即時深淺色對照。

- 🗂️ **庫存總覽持股表格分組列通欄純章節化**（`DashboardPage.tsx`、`DashboardPage.test.tsx`）—
  - 移除多空分組列後方之市值與損益小計（已由上方 Summary 卡片直觀呈現），將標題單元格調整為 `colSpan={10}` 通欄跨滿，建立簡潔清晰的段落視覺層次。

- 💰 **未實現損益金額與報酬率雙行直顯券商 APP 牌告口徑**（`holdingRows.ts`、`DashboardPage.tsx`、`QuoteTab.tsx`）—
  - 主標顯示實質折讓後淨損益與報酬率，副標（11px 灰字）直顯 `券商 -NT$XX` 與 `券商 XX%`（依台灣法定牌告 0.1425% 精算），1:1 完全對齊券商 APP 月退制口徑，消除每檔數百元與約 0.10% 的退佣差額疑慮；未設折讓自動隱藏。

- 📐 **觀察股票卡片迷你緊湊化微調**（`index.css`）—
  - 網格寬度收縮至 `minmax(136px, 1fr)`，高度由 72px 壓至 58px（減少約 20% 高度），字級與間距精準微調，大幅釋放儀表板垂直空間。



### 0.9.29（2026-09-03）— 觀察股票同產業自動群組聚合、緊湊型小卡、MIS 即時產業別與收盤無成交價格修復 (BUG-045, BUG-046)

> 這一版跨三次迭代（0.9.29-dev.1 ~ dev.3），主線是**觀察股票產業自適應聚合與即時資料流優化**：觀察清單同產業標的自動成組並提供膠囊快選，重構緊湊型雙行卡片大幅釋放空間；同時打通 TWSE MIS 官方 33 大產業代碼即時資料流與個股分析行情卡產業標籤，並修復收盤無成交價格與字典誤植（BUG-045, BUG-046）。

- 🏷️ **觀察股票同產業自動聚合群組（Auto-Grouping）與膠囊快選**（`stockGrouping.ts`、`WatchSection.tsx`、`index.css`）—
  - 核心邏輯：依據個股解析之官方/推導產業別（`getStockCategory`），當同產業標的數 $\ge 2$ 時（例如加入長榮 2603 與陽明 2609 同為「航運業」），自動將其聚合成獨立產業族群（如「航運業 (2)」）；單一標的或未分類者歸於「其他」。
  - 產業名稱規範化（`getGroupCategoryName`）：建立 `CANONICAL_INDUSTRY_MAP`，將簡短徽章標籤（如「航運」、「半導體」）與官方產業全名（「航運業」、「半導體業」）無縫對齊，特別資產（ETF、ETN、特別股）保持原樣。
  - 膠囊列（Filter Chips）：於圖卡與條列模式頂部動態提供「全部 (N)」、「產業 (M)」、「其他 (K)」一鍵切換膠囊，快速聚焦特定族群；切換檢視模式時篩選狀態無縫維持。
  - 分組標題：圖卡模式下依群組呈現微型標籤與計數；條列模式下以群組列（`watchlist-group-row`）清楚劃分區塊；當標的移除使得群組數量 $< 2$ 時，自動解構回復無分組佈局。
  - 篩選崩潰防護：在「其他」篩選狀態下若刪除最後一檔其他標的，`activeFilter` 安全退階為「全部 (N)」並自動同步重設 `filter` state，消除畫面全空問題。
- 🧭 **個股分析頂部切換選單產業分組**（`AnalysisPage.tsx`）—
  - 頂部「切換個股」下拉選單（`HeaderMenu`）的觀察股票區塊，依據相同產業自動分組顯示（如「觀察 ── 航運業」、「觀察 ── 半導體業」、「觀察 ── 其他」），方便使用者按族群快速挑選切換。
- 🗂️ **觀察股票緊湊型小卡（方案 A）**（`WatchSection.tsx`、`index.css`）—
  - 網格改為 `repeat(auto-fill, minmax(165px, 1fr))`，卡片內距緊湊化（`10px 12px`），卡片高度壓縮至 ~72px（減少約 35%~40% 垂直佔位）。
  - 雙行佈局：第 1 行整合股票代號、名稱、微型分類標籤與右上角移除按鈕；第 2 行對齊現價（18px，Tabular Nums 與紅綠配色）與漲跌幅百分比；條列檢視同步在名稱旁展示微型分類徽章。
...
- 🏷️ **台股類型與產業即時判定**（`stockCategory.ts`）— 0ms 純函數即時推導標籤，涵蓋：
  - 規則式類型：`00...B` 債券 ETF、`00...L` 槓桿 ETF、`00...R` 反向 ETF、`00...` 股票型 ETF、`02...` ETN、`91...` TDR、`01...` REITs、代號含英文字母（如 `2881A`）特別股。
  - 官方產業分類：建立 350+ 檔常見上市櫃股票之 33 大產業字典對照（如 2330 半導體、2603 航運、2881 金融保險等），並支援公司名稱關鍵字啟發式後備比對。
- 🧪 **完整驗證** — 全套 97 檔測試檔案、1599 個測試 100% 通過；`npm run typecheck:edge`、`npx oxlint src` (0 error) 與 `npm run build` 均 exit 0。


### 0.9.28（2026-09-03）— 融券做空、全站冷處理改版與表格可讀性

> 這一版跨十次迭代，主線是**跨日做空**：過去融券賣出會被當成超賣，成本以 0 計，
> 已實現損益等於整筆賣出金額。空頭現在是獨立部位，多頭路徑一行未動。

- 🩳 **融券做空**（`pnlEngine.ts`、`models.ts`、`fees.ts`）— `TxNature` 新增 `'SHORT'`，
  `Position` 新增 `shortQty` / `shortProceeds` / `shortRawProceeds` / `shortLots` 四個**必填**欄位。
  建倉不碰 `pos.qty` 與 `openLots`；回補以 FIFO 消耗空頭批次，`已實現 = 賣出淨收 − 回補總成本`。
  年度口徑只在回補時記帳，否則「總賣出金額」會被尚未實現的交易灌水。超額回補視為現股買進並發警告。
  借券費 0.08%（`BORROW_FEE_RATE`），`splitFeeTax` 回傳第三個分量 `borrow`。
  **融券賣出一律付全額證交稅**——資券當沖不適用減半。
- ⚡ **現股當沖先賣後買撮合**（`pnlEngine.ts`）— 按交易日分群並優先撮合同日當沖，扣除手續費與
  0.15% 減半證交稅後精算損益。既有波段持股的均價與未實現損益 100% 不受影響，對齊券商對帳單行為。
- 🐛 **當沖撮合會吃掉融券腿** — 候選清單在 fallback 分支會擴大成當日全部反向交易，導致融券賣出被
  當沖撮合掉、空單根本不會建立。候選過濾現已明確排除 `SHORT`。
- 📊 **儀表板多空並列** — 一檔同時有波段持股與空單時產出兩列，持股表改為方向分組並帶段首小計，
  淨額市值搬到市場抬頭並配一條比例曝險尺。**兩條腿都要有價格，淨額才成立**：任一腿無報價時顯示
  骨架，而不是把該腿當成 0——否則會印出一個看起來合理的錯誤淨額。`投入總成本`只算多頭列，
  多空同檔的成本只算一次。
- 🧊 **全站冷處理改版** — 移除所有 `backdrop-filter`，圓角 14px → 2px、卡片陰影歸零；紫藍換成鋼藍，
  刪除所有漸層與光暈。新增 `--steel` / `--ochre` 兩組語意色：紅綠只標漲跌，鋼藍標空方與信用交易，
  琥珀標當沖，三者不出現在同一欄位。
- 🧾 **交易類型合併成單一色塊** — 文字含性質與買賣（`現股買`、`融券賣`）。`tx_nature` 為 null 代表
  **未知而非現股**，此時只顯示「買入／賣出」並用中性色塊。現金收支欄不再上漲跌色——買進恆負、
  賣出恆正，舊的染色只是在重複「買／賣」兩個字。
- 🔤 **字體與字級回到正式區基準** — 中途換過 Archivo + IBM Plex Mono 與 Roboto，兩者都是誤判：
  git 歷史顯示初版就是 Outfit + Inter。以 Playwright 同時量測兩站後還原，並移除 `--font-num`。
  字級亦逐條比對還原（`.section-title h2` 等四條）。
- 🧮 **只有空單時的兩個缺陷** — 總覽整片變骨架灰條，因為 `sumOrNull` 對空陣列回傳 `null`，而 `null`
  在面板裡代表「報價還沒到」；沒有多單時多方腿被當成未知而非 0。籌碼分析回 403，因為白名單用
  `net = 買進 − 賣出 > 0` 判定持有，融券先賣後買、淨額必為負。前者在來源處分開零與未知，
  後者判定改為 `net !== 0` 並抽成 `netOpenTickers()`。
- 🎚️ **表格底色分層**（25 個表格共用 `.data-table`）— 逐像素取樣後確認暗色主題的頁面底、卡片、
  標題列三層只跨越 5.14 個 L\*。`--thead-bg` 暗色 0.03 → 0.09、亮色 0.035 → 0.08，資料列不動，
  明度差由 +3.15 拉到 +10.06。表頭文字改用 `--ink-secondary`——原本在暗色 3.85:1、亮色 3.03:1、
  PDF 匯出 2.86:1，**三者都低於 WCAG AA 的 4.5:1**，改後全部通過。矩陣表凍結首欄改為跟著所屬的
  列走，修掉亮色下首欄與上下都對不上的既有缺陷。
- 🗄️ **Schema** — `transactions.tx_nature`（含 `'SHORT'`）、`transactions.fee_rate`、
  `workspaces.fee_rate`。DEV 與 PROD 皆已套用，DDL 為純新增且可為 NULL，既有資料未被改寫。
- ✅ 測試 1490 → **1537**，95 檔全過。`npm run build`、`npm run typecheck:edge`、`npx vitest run`
  皆 exit 0；`npx oxlint src` 0 error。

### 0.9.27（2026-09-01）— 個別交易手續費率持久化、庫存參考淨值雙層呈現、批次持股專屬費率估算與股票分割換算精靈

- 📊 **儀表板庫存參考淨值與批次專屬費率估算**（`holdingRows.ts`、`pnlEngine.ts`、`DashboardPage.tsx`、`QuoteTab.tsx`）—
  - `HoldingRow` 新增 `netMktVal` 欄位（扣除預估賣出手續費與證交稅後的預估實收金額），於表格「目前市值」下方以小字副標顯示 `淨收 {fmtMoney}`，與成本、均價、未實現損益的雙行排版風格一致，方便與券商 APP「參考淨值／預估贖回款」直接 1:1 對帳。
  - `OpenLot` 擴充 `feeRate` 屬性，在計算未實現損益預估賣出費用時，優先使用該批買入部位自帶之獨立費率（如牌告 0.1425% 或 3 折 0.04275%），解決多券商混用時全域預設費率干擾各別持股損益的問題。
  - 在「損益兩平價」增補 Tooltip 說明標的精算稅率（ETF 0.1% vs 一般股票 0.3%）。
- ✂️ **互動式股票分割換算精靈**（`StockSplitModal.tsx`、`TransactionsPage.tsx`）— 於交易紀錄頁面工具列新增「股票分割換算」按鈕，支援「1 拆 N 股票分割」與「N 併 1 反向分割／減資」：
  - 支援下拉選取標的、設定比例、選填基準截止日。
  - 即時提供換算前後之總股數、平均買價、總投入成本與受影響明細預覽卡。
  - 零資料庫 Schema 變更風險，一鍵批次精確更新歷史買入單據。
  - **手續費智慧鎖定與補算**：精靈自動識別歷史交易手續費率並鎖定保護；原紀錄為 0 時可自動依折讓費率帶入，保持歷史交割總成本恆定。
- 💾 **交易手續費率欄位擴充與歷史智慧反推**（`schema.sql`、`models.ts`、`dataProvider.ts`、`fees.ts`、`TransactionForm.tsx`）—
  - 在 `transactions` 資料表新增 `fee_rate NUMERIC` 欄位，資料層維持舊版 Schema 降級防禦。
  - `fees.ts` 新增 `inferFeeRate` 輔助函式，針對 `fee_rate` 為空值的歷史紀錄，依扣稅後成交額自動比對台股常見折數反推實質折數。
  - 編輯交易表單優先使用該筆紀錄之專屬費率等比重算，避免手動編輯時手續費失真。
- 🧪 **完整驗證** — 單元測試 95 檔 / 1485 個測試 100% 通過，`tsc -b && vite build` exit 0。
...
> 一次發版收攏 Task 136、137、138：未實現損益改為逐批計算以對上券商 APP、當沖的稅金與手續費正確拆分、交易性質成為可儲存可匯出的欄位。修好 BUG-033 至 BUG-039 共七項缺陷，撤銷 BUG-040。
>
> 同日另外修好一件與程式碼無關的基礎設施故障：兩個 Supabase 專案的 12 個 cron job 從建立起就沒運作過，詳見最後一則。

- 📊 **逐批未實現損益**（Task 136）— `Position` 與 `Holding` 新增 `openLots: OpenLot[]`；`computeLedger` 買進時新增一批、賣出時依 FIFO 消耗批次；`estimateUnrealized` 改為**逐批**對賣出手續費與證交稅無條件捨去，而非整筆一次捨去。對照案例（0050 四批 2000/1000/1000/2000 股、成本 625,188、現價 106.25）：逐批 10,770 元，整筆 10,767 元 —— 這 3 元就是與券商 APP 對不起來的差額。兩項與規格書不同的取捨已記在規格檔：成本項仍用整筆移動平均（避免部分賣出後未實現損益與已實現損益、報酬率分母脫鉤），最低手續費對整筆計一次（清倉是一張賣單，只收一次最低手續費）。
- 🧾 **當沖稅金拆分**（BUG-036）— `computeLedger` 過去用 `Math.min(標準稅, 總金額)`，當沖賣出的標準稅高於實收總額，手續費因而被壓成 0。改為稅金級梯：實收金額足以覆蓋標準稅時用標準稅，否則用減半稅（現股當沖），再不足才全數計為稅。2344 華邦電 2026-08-18 那筆現在正確拆為稅 282 元、手續費 80 元。
- 🛡️ **批次重算不再覆蓋當沖紀錄**（Task 137）— `proposeFeeCorrections` 排除疑似當沖的賣出，判定規則為「實收金額低於標準稅」**且**「扣掉減半稅後的餘額恰等於應收手續費」。**不使用「同日買賣」判斷**：兩份真實券商匯出檔共 40 筆賣出中有 14 筆是當日進出，其中只有 2 筆是當沖，單用日期比對會產生 12 筆誤判。批次重算視窗的預設勾選維持原樣（仍為全選），另加一行提醒；真正的破壞性提案已由上述排除規則移除。
- ✏️ **編輯交易時不再靜默改寫原紀錄**（BUG-033）— 開啟編輯視窗會立刻依現行費率重算並覆蓋 `initial.fee_tax`。改為比對目前輸入與初始值：只要沒有變動就不重算。**不用「跳過第一次執行」的旗標** —— `main.tsx` 以 `StrictMode` 掛載，effect 在掛載時會跑兩次，第二次就會把旗標吃掉並照樣覆蓋（實測會得到 833 而非 362）。
- 🏦 **TDR 與 REITs 證交稅率**（BUG-035）— `sellTaxRate` 補上 TDR（`91xx`）與 REITs（`01xxx`）的 0.1%。債券 ETF（`00xxxB`）仍為 0%，ETF（`00xx`）仍為 0.1%，其餘一般股票仍為 0.3%；`9910`、`9945` 是一般上市公司，維持 0.3%。
- 💯 **百分比進位**（BUG-034）— `fmtPercent` 改為對絕對值做十進位四捨五入後再補回正負號（`Math.round` 的中間值往 +Infinity 進，直接加 `Number.EPSILON` 會讓負值少進一位）。防呆條件由 `Number.isNaN` 改為 `!Number.isFinite`，`Infinity` 現在顯示「—」。
- 💰 **零成本持股的保本價**（BUG-038）— `breakEvenPrice` 原本對 `cost === 0` 直接回傳 0。現在會算出足以覆蓋賣出手續費與證交稅的最低價（1000 股、`minFee` 20 時為 0.02）。
- 🧮 **清倉後的浮點殘值**（BUG-039）— 移動平均反算在清倉後留下約 1.4e-11 的殘值，會永久污染下次買進的均價。數量歸零時改為明確清除 `cost`、`rawCost` 與 `openLots`。
- 🔢 **會計負數括號**（BUG-037）— `parseNumber` 現在能解析 `(1,000)`。**使用者看不到差別**：這個函式是模組私有，三個呼叫端都拒絕負數，修正前後同樣被擋下。
- 🚫 **BUG-040 撤銷** — 「管理員可無確認地撤銷自己的權限」不成立，後端 `handleAdminSetRole` 已經擋下，既有測試也已鎖住此行為。未改任何程式碼。
- 🧪 **測試** — 新增 `sources/src/utils/realExports.test.ts`，把 `docs/` 下兩份真實券商匯出檔跑過 `parseTransactionsCsv` → `computeLedger` → `proposeFeeCorrections`，以 106 筆真實資料鎖住當沖判定。全套由 91 檔 / 1377 測試增為 **92 檔 / 1416 測試**，全數通過；`npx tsc --noEmit` 與 `npm run build` 均 exit 0。
- 📋 **未納入本版** — Task 137 §C（CSV `交易性質` 與分項手續費／證交稅欄位）需新增交易欄位，也就需要 Supabase 遷移，而 PROD schema 變更卡在 BUG-041。另有兩筆真實紀錄異常待人工核對券商對帳單：`2026-06-05 2891` 記 3,932 元而計算值為 722 元，`2026-06-23 00685L` 記 459 元（原價費率），但同檔其他紀錄皆為 3 折。
- ⚠️ **新增 RISK** — `breakEvenPrice` 在 `cost === 0` 且未給 `minFee` 時回傳 0，與「無解」的哨兵值相同。美股持股會走到這條路徑。**既有問題，非本次引入**。
- 🚀 **不需部署** — `sources/supabase/functions/` 無變更。
- 🗄️ **交易性質欄位** — `transactions.tx_nature TEXT` 加上 CHECK 約束（NULL / SPOT / DAY_TRADE / MARGIN），寫入 `sources/supabase/schema.sql`，採用與 `workspaces.fee_rate` 相同的冪等寫法。DEV 已套用並驗證：欄位存在、約束實測拒絕非法值 `FUTURES`、PostgREST 對 `select=id,tx_nature` 回 HTTP 200（欄位不存在時會是 400）。**PROD 未套用**，SQL 記在 BUG-041。
- 🏷️ **型別與標籤**（`types/models.ts`）— 新增 `TxNature`（`'SPOT' | 'DAY_TRADE' | 'MARGIN'`）與 `TX_NATURE_LABEL`（現股／當沖／融資），`Transaction` 新增選填的 `tx_nature`。**選填是刻意的：NULL 代表「未知」，不是「現股」** —— 欄位存在之前寫入的紀錄，以及 PROD 在遷移前的每一筆，都必須繼續走推測。
- 🛡️ **資料層降級，且重試刻意收斂** — `listTransactions`、`addTransactions`、`updateTransaction` 在資料庫尚未有該欄位時各退回一次舊欄位清單。**重試只在「欄位不存在」（`42703` 或 `PGRST204`）時觸發**，其他錯誤立即拋出：INSERT 不具冪等性，若第一次其實已寫入、只是回應失敗，盲目重試會寫出重複交易並靜默污染部位成本。
- ⚖️ **費稅拆分集中化**（`utils/pnlEngine.ts`）— 新增匯出的 `splitFeeTax`，把原本寫在 `computeLedger` 裡的拆分邏輯抽出來，供 CSV 匯出共用同一套規則。明確標記 `DAY_TRADE` 直接採信（減半稅，不推測）；`SPOT`、`MARGIN`、NULL 與未標記一律沿用推測階梯。**明確標記只能增加資訊，不能倒回 BUG-036**：若讓 `SPOT` 強制一般稅率，一筆被誤標的當沖紀錄會再次被 `Math.min` 把手續費壓成 0。`proposeFeeCorrections` 則在標記為當沖時直接跳過，不再跑推測。
- 📄 **CSV 分項欄位與交易性質** — 匯出表頭增為 `交易日期,市場,股票代號,股票名稱,交易類型,交易性質,交易單價,交易股數,手續費,證交稅,手續費 / 稅金`，保留舊的合併欄位讓舊版仍讀得懂。匯入接受中文標籤與英文代碼，分項欄位相加寫回 `fee_tax`，無法辨識的性質逐列回報。**欄位比對順序是關鍵**：`'手續費'.includes('手續費')` 為真，分項欄位必須先用精確比對且兩欄同時存在才算數，否則會被舊的子字串比對當成合併總額。沒有新欄位的檔案行為完全不變。
- 🎚️ **表單選擇器**（`TransactionForm.tsx`）— 新增 `交易性質` 選擇器，僅台股顯示。選「當沖」會一併把證交稅率帶成 0.0015，也就是使用者本來要手動改的那一步。
- 🧪 **測試** — 全套由 92 檔 / 1416 測試增為 **93 檔 / 1450 測試**，全數通過，`npm run build` exit 0。`realExports.test.ts` 另外把兩份真實券商匯出檔（106 筆）用新格式匯出再匯入，逐筆確認金額、股數、代號與買賣別不失真，且分項的手續費加證交稅每一筆都等於原本的總額。
- 🔍 **審查與主 session 的兩項攔截** — 計算層 PASS 無發現；資料層 PASS 附一項 RISK。另有兩個缺陷是主 session 讀 diff 時攔下、送審之前就修掉的：(1) 重試原本對任何錯誤都觸發，會在非冪等的 INSERT 上寫出重複交易；(2) `splitFeeTax` 原本讓 `ticker` 選填並在缺少時退回 0.3%，會讓每一檔 ETF、TDR 與 REIT 靜默多課三倍的稅——改為必填，交給編譯器擋。
- ⚠️ **已知風險（RISK-004）** — PROD 遷移之前，`addTransactions` 會把 `tx_nature` 剝掉後重試並回報成功，使用者選了「當沖」卻在重開時發現標記不見了。**已接受，不改程式**：帳簿的推測階梯對真實的當沖紀錄仍算出正確的稅金拆分，數字不受影響，遺失的只有明確標記，且在 PROD 跑完遷移後就消失。與 Task 135 的 `workspaces.fee_rate` 是同一個特性。
- 🚀 **不需部署** — `sources/supabase/functions/` 無變更。
- 🔧 **基礎設施：批次工作恢復運行**（非程式碼變更）— 兩個雲端專案在 2026-08-31 重建時，設定 SQL 的兩個佔位符都沒有被替換：cron 指令的 URL 留著字面的 `<PROJECT_REF>`，標頭留著字面的 `<CRON_SECRET>`。前者讓 `net.http_post` 在排入佇列前就失敗，後者讓 Edge Function 回 401。**12 個 job 從建立至今零次成功**。已就地替換 URL 並為兩個專案各輪替一組新的 43 字元 `CRON_SECRET`（Edge secret 與 cron 指令同步更新，值全程未離開資料庫與 CLI）。驗證：13:30 為 401、13:35 為 **HTTP 200**，`cron.job_run_details` 轉為 `succeeded`。舊的專案代號 `kxnxadaghidwumqsqneu` 與 `cahmfrhacyvrrlsaatkm` 皆已刪除，`CLAUDE.md` 的環境表已更正。
...
- 🔐 **帳號內變更密碼**（`context/AuthContext.tsx`、`components/AppShell.tsx`）— 使用者選單新增「變更密碼」，本機模式隱藏。新增的 `changePassword()` **先以舊密碼呼叫 `signInWithPassword` 重新驗證**，因為 Supabase 允許任何存活的 session 直接 `updateUser({ password })`，不需要任何舊密碼的證明；舊密碼錯誤回傳「目前密碼不正確」且不會進到 `updateUser`。經查證 `auth-js`：重新驗證失敗時它在 `_saveSession` 與 `_notifyAllSubscribers` 之前就回傳，所以輸錯舊密碼不會破壞現有 session。成功後保留 modal 顯示「密碼已變更」、清空欄位並鎖住送出鍵——關閉 modal 會讓使用者看不到確認訊息，而未鎖住的送出鍵會拿已變更的密碼去重新驗證而失敗。
