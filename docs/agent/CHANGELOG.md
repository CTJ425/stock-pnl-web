...
### 0.9.25-dev.2（2026-09-01）— 交易性質欄位、費稅分項與 CSV 往返

> 補完 Task 137 §C。上一版把 §C 判為超出範圍是錯的判斷，本版推翻並完整實作。DEV schema 已套用並驗證；PROD 遷移與 0.9.25-dev.1 的費率欄位一併記在 BUG-041。
...
### 0.9.25-dev.1（2026-09-01）— 逐批未實現損益、當沖稅金拆分、編輯表單不再覆蓋原紀錄

> 完成 Task 136、137、138，修好 BUG-033 至 BUG-039 共七項缺陷，並撤銷 BUG-040（前提不成立）。
