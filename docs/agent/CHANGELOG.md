...
- 🧮 **歷史舊資料智慧反推**（`fees.ts`）— 新增 `inferFeeRate` 輔助函式，針對 `fee_rate` 為空值的歷史紀錄，利用 `(fee_tax - 證交稅) / (price * qty)` 扣稅後比對台股常見折數（原價、6.5 折、6 折、5 折、4 折、3.8 折、3 折、2.8 折、2.5 折、2 折等），精準反推建立時的實質折數；若遇低消限制（如整股 20 元）則回退預設費率，防止反推畸高比率。
- ✏️ **編輯表單手續費率對齊**（`TransactionForm.tsx`）— 開啟編輯時不再強制以工作區全域預設費率覆寫，而是優先使用該筆交易記錄之 `fee_rate`（或歷史反推費率）；編輯修改單價或股數時，自動以該筆交易專屬的折讓費率等比重算，徹底解決手動編輯交易時手續費失真的問題。
- 🛡️ **降級路徑修正** — 舊 Schema 降級改為「只剝除錯誤訊息指名的那一個欄位」。先前版本會連同 `tx_nature` 一併剝除，在「有 `tx_nature`、缺 `fee_rate`」的資料庫（即目前 PROD 狀態）會讓每筆寫入的 `tx_nature` 變成 NULL，靜默破壞當沖證交稅減半的判定。重試維持僅由 42703／PGRST204 觸發，最多 3 次。
- 🧮 **反推費率上限修正** — `inferFeeRate` 改為必須接收工作區的整股／零股低消，不再比對寫死的 20／1；比值上限由 0.05 收緊至法定的 `DEFAULT_FEE_RATE`。實測：零股低消 5、成交 1,000、手續費 5，原本反推出 0.005（法定值 3.5 倍），改股數後手續費算成 500，正確值為 142。
- 🧪 **完整驗證** — 單元測試 94 檔 / 1476 個測試 100% 通過，`npm run build` exit 0。
- ⚠️ **E2E 測試範圍說明** — `scripts/verify-fee-rate-e2e.cjs` 在攔截全部 `**/rest/v1/**` 的前提下，驗證「自訂 3 折費率建立 → 送出 payload 帶入費率 → 重開編輯保留費率 → 修改單價依 3 折重算」。**此測試不連線資料庫**，資料庫欄位是否就位另以 `verify_setup()` 確認（DEV 已 10/10 PASS）。
