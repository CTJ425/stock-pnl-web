...
### 0.9.28-dev.10（2026-09-03）— 表格底色分層

- 🎚️ **表格標題列與資料列拉開明度差**（`index.css`）—— 原本標題列與資料列幾乎同色。
  逐像素取樣後確認根因：暗色主題的頁面底、卡片、標題列三層**只跨越 5.14 個 L\***，
  標題與資料列之間僅 +3.15，正好落在人眼分辨門檻上。`--thead-bg` 暗色由 `0.03` 提到
  `0.09`、亮色由 `0.035` 提到 `0.08`，**資料列完全不動**。暗色 ΔL\* 由 +3.15 拉到 +10.06，
  亮色由 −2.78 拉到 −6.60。全站 25 個表格共用同一條規則，一次到位。
- ♿ **表頭文字改用 `--ink-secondary`** —— 原本的 `--ink-muted` 在暗色 3.85:1、亮色 3.03:1、
  PDF 匯出 2.86:1，**三者都低於 WCAG AA 的 4.5:1**，是既有問題。改後分別為 5.73、6.47、5.58，
  全部通過。
- 🔧 **矩陣表凍結首欄跟著所屬的列走** —— `.inst-matrix` 的首欄原本表頭格與資料格共用
  `var(--panel)`。亮色主題下那是 `#ffffff`，而表頭是 `#f2f3f4`、資料列是 `#fafbfc`，
  首欄與上下兩者都對不上。改為表頭格用新的 `--thead-solid`、資料格用 `--surface`，
  7 個矩陣表都已對齊。
- 🧷 **分組列與 PDF 匯出面維持原樣** —— `.holding-group td`、`.adm-prompt-locked` 原本借用
  `--thead-bg`，改指向新的 `--row-band` 以保留舊色調；`.report-surface` 自帶調色盤，底色不變。
- ✅ 驗證全部以像素實測，非計算推估。`npm run build`、`npx vitest run` 皆 exit 0，
  95 檔 1537 測試；`npx oxlint src` 0 errors。
...
- 📊 **曝險尺永遠顯示**（`DashboardPage.tsx`）— 沒有空單時空方段為 0，不再整條收掉。
  唯一的例外是「有空單但整條腿抓不到報價」：那時 0 是未知不是零，畫出來的比例是假的，
  仍然不畫（T14 守著）。美股面板因此也有尺，空方恆為 0。
- 👁️ **空單列看得到建倉價**（`DashboardPage.tsx`）— 空單的「投入成本」與「平均買入成本」
  兩欄原本印「—」，等於看不到自己在幾塊放空、收了多少，而那正是判斷要不要回補時唯一需要的
  數字。改為顯示 `shortProceeds`（實收，已扣手續費、證交稅與借券費）與
  `shortProceeds / shortQty`（建倉均價），副行給未扣費的成交價金與成交均價。
  引擎未改動——這兩個欄位 Task 141 就已經存在，只是沒有畫出來。
- 🔤 **字體改回 Roboto**（`index.html`、`index.css`）— Archivo + IBM Plex Mono 換成
  Roboto + Roboto Mono，維持 MUI 的預設字族。只換字體，互動與版面結構一律未動。
- 🩹 **表格溢出修正** — 新增的空單副標把「投入成本」欄撐到 170px，整張台股表溢出 66px，
  「未實現報酬率」被推出容器。副標縮短為「賣出・未含費」，表格左右內距 10px → 8px。
  量測結果：溢出 0px。
- ✅ 測試 1530 → 1531（新增 T15 空單建倉價；T3 / T4 改寫為曝險尺常駐的新契約）。
  `npm run build` 與 `npx vitest run` 皆 exit 0。

### 0.9.28-dev.6（2026-09-03）— 曝險尺圖例分組

- 🏷️ **圖例改成群組**（`DashboardPage.tsx`、`index.css`）— 原本色塊、標籤與數字六個元素平均
  以 16px 排開，色塊在視覺上不屬於它的標籤。改為每組一個 `.exposure-legend`：組內 7px、
  組間 16px，與設計一致。
- 📐 **曝險尺本身經量測確認正確**：521×8px、兩段間距 2px、多方段 `--ink`、空方段 `--steel`，
  比例與市值相符。**沒有融券部位時刻意不渲染** —— 單段的比例尺不表示任何比例。
  這是 Task 142 spec C2 的負向條件。
- ✅ 測試 1530 筆不變，`npm run build` 與 `npx vitest run` 皆 exit 0。

### 0.9.28-dev.5（2026-09-03）— 修正市場抬頭：主數字與曝險尺沒有照設計做

> 0.9.28-dev.4 宣稱「底部欄位用 `margin-top: auto` 推到底，兩塊面板永遠齊平」。**那句話是錯的。**
> `.market-panel .metric-row { margin-top: 12px }`（優先權 0,2,0）壓過 `.market-foot { margin-top: auto }`
> （0,1,0），auto 從未生效。這一版把抬頭整塊按原設計重做。

- 📐 **主數字標籤移到抬頭列右端**（`DashboardPage.tsx`、`index.css`）— 原本「淨額市值」自成一行、
  壓在一條分隔線下面；現在與「台股 TWD」同一行、靠右對齊（`margin-left: auto`），抬頭下方的分隔線移除。
- ➕ **淨額帶正負號** — 有空單時主數字改用 `fmtSignedMoney`，因為多空相減可能為負。
  沒有空單時是單純的持倉市值，維持不帶號。
- 🩹 **底部欄位真的推到底了** — `.market-foot` 提升為 `.market-panel .market-foot`（同優先權且在後），
  並改成滿版出血（左右 `-20px`）、上方一條分隔線、兩欄之間一條中線。
  量測結果：兩塊面板 `footBottom` 皆為 317px。
- 🏷️ **用語對齊設計** — 曝險尺圖例與表格分組標題由「多方 / 空方」改為「多單 / 空單」。
- 📏 面板內距由 `14px 16px 16px` 改為 `15px 20px 0`。
- ✅ 測試 1530 筆不變，`npm run build` 與 `npx vitest run` 皆 exit 0。
  Playwright 量測：持股表溢出 0px、主數字帶號、標籤在抬頭列。

### 0.9.28-dev.4（2026-09-03）— 全站冷處理：拆毛玻璃、換字體、鋼藍取代紫

> 0.9.28-dev.3 做了結構與顏色分工，但沒動視覺處理，所以看起來還是原本的樣子。這一版是視覺處理。
> 只改 `index.css` 與 `index.html`，沒有動任何 `.tsx`、任何類別名稱、任何使用者可見字串。

- 🧊 **毛玻璃全數移除**（`index.css`）— 刪掉所有 `backdrop-filter`，`--radius` 14px → 2px、
  `--radius-sm` 9px → 2px、`--shadow-card` → `none`。三個 token 一改就覆蓋 100 多條規則。
  `border-radius: 50%` 與 `999px` 刻意不動——那是頭像與圓點，改了會把圓形變方形。
- 🎨 **紫藍換成鋼藍**（`index.css`）— `--accent` / `--accent-strong` / `--accent-2` 改為鋼藍。
  `.kpi::before` 與 `.market-panel::before` 那兩條 2px 漸層線刪除；`.tab.active` 的漸層藥丸換成
  平面文字加 2px 底線；`.btn-primary`、`.fab`、頭像、`.adm-toggle.on` 的漸層與紫色光暈一併拆掉。
  背景兩圈 radial 光暈設為 `transparent`。
- 🔤 **字體換成 Archivo + IBM Plex Mono**（`index.html`、`index.css`）— 新增 `--font-num`，
  只套在 `.data-table .num`、`.kpi-value`、`.kpi-sub` 三處並開 `tabular-nums`。內文不套，
  中文不會落進 Latin 等寬字體。
- 📏 **表格加密**（`index.css`）— 表頭 10.5px 全大寫、列高壓縮、分隔線降到 4.5%（新增 `--rule-hair`）。
- 🐞 **首次算圖抓到四個缺陷，都已修**：
  - 字距 0.14em / 0.18em 是給拉丁標籤用的，套在中文上變成「交 易 日 期」。降到 0.02em / 0.06em。
  - 等寬字比 Inter 寬，台股十欄表被擠出容器。關鍵修法是把儲存格內的副行（未含費…、淨收…）改回
    比例字體——那是文字，不是需要對齊的欄。
  - `.btn-primary` / `.fab` / 頭像 / `.adm-toggle.on` 四處紫色漸層在第一版沒清到。
  - `.data-table` 的 `th` 與 `td` 左右內距不一致（9px vs 14px），統一為 10px。
- ✅ 測試 1530 筆不變，`npm run build` 與 `npx vitest run` 皆 exit 0。深淺主題各截圖驗證。

### 0.9.28-dev.3（2026-09-03）— 持股表冷處理：分組小計、淨額上移、色塊取代線框

> 表尾固定印出「多頭市值／空頭市值／淨曝險」三行，但三行只有在同時做多做空時才互不相同。
> 這一版把它們拆掉，改成表格內分組小計加抬頭淨額。金額算式一行未動。

- 📊 **持股表改成方向分組**（`DashboardPage.tsx`）— 刪除「方向」欄與整個 `<tfoot>`（含
  `totals-long` / `totals-short` / `totals-net` 三個 testid）。多單、空單各成一段，段首列帶檔數與
  小計市值、小計未實現。段首不算分組報酬率——空頭分組的報酬率不是單純加總，寧可留白也不給錯的數字。
  沒有空單時完全不渲染段首列。
- 🧮 **淨額搬到市場抬頭**（`DashboardPage.tsx`）— 兩塊 `market-panel` 抽成同一個 `MarketPanel` 元件，
  台美股仍舊並排。有空單時主數字是「淨額市值」＝多方 − 空方，並畫一條比例曝險條；沒有空單時主數字
  是「持倉市值」，不畫條也不畫圖例。**兩條腿都要有價格，淨額才成立**：任一方全無報價時顯示骨架而
  非把該腿當成 0，否則會印出一個看起來合理的錯誤淨額，並讓曝險條變成 100/0 的單段條。
  `投入總成本`仍只算多頭列，多空同檔的成本只算一次（0.9.28-dev.2 的保證不變）。
- 🎨 **色塊取代線框**（`index.css`、`TransactionsPage.tsx`）— 新增 `--steel` / `--steel-tint` /
  `--steel-on` / `--ochre` / `--ochre-tint` 五個 token，深淺主題各一份。紅綠仍然只標漲跌；
  鋼藍標空方與信用交易，琥珀標當沖。三組顏色不出現在同一個欄位，所以不會互相解讀錯。
  空單列鋪淡鋼藍底並在最左帶一根實色標記。
- 🧾 **交易類型合併成單一色塊**（`TransactionsPage.tsx`）— 類型欄從「買入／賣出」改成一顆色塊，
  文字含性質與買賣（`現股買`、`融券賣`）。`tx_nature` 為 null 代表**未知而非現股**，此時只顯示
  「買入／賣出」並用中性色塊；美股交易一律不寫 `tx_nature`，走的就是這條路徑。
- 🐞 **現金收支欄不再上漲跌色**（`TransactionsPage.tsx`）— `cashFlow` 只算 `單價 × 股數 ∓ 費稅`，
  買進恆負、賣出恆正，所以舊的 `pnlClass(flow)` 把每一筆買進染綠、每一筆賣出染紅，顏色只是在重複
  「買／賣」兩個字，還在台股讀法下說「買進都是跌」。整欄改為中性，欄名由「損益 / 收支」正名為
  「現金收支」。
- ✅ 測試 1518 → 1530，`npm run build` 與 `npx vitest run` 皆 exit 0。

### 0.9.28-dev.2（2026-09-02）— 融券做空：空頭部位、借券費與多空並列儀表板

> 現股當沖的先賣後買在 0.9.28-dev.1 已經可用，但**跨日做空**過去完全算錯：融券賣出會被當成超賣，
> 成本以 0 計，已實現損益等於整筆賣出金額。這一版把空頭做成獨立部位，多頭路徑一行未動。

- 🩳 **融券空頭部位**（`pnlEngine.ts`、`models.ts`）— `TxNature` 新增 `'SHORT'`（融券），`Position` 新增
  `shortQty` / `shortProceeds` / `shortRawProceeds` / `shortLots` 四個**必填**欄位。
  - **建倉**：`SHORT` + 賣出建立空頭批次，不碰 `pos.qty`、`pos.cost` 與 `openLots`，也不再誤發超賣警告。
  - **回補**：`SHORT` + 買進以 FIFO 消耗空頭批次，`已實現 = 賣出淨收 − 回補總成本`（回補不課證交稅）。
  - **年度口徑**：建倉不計入任何金額欄位，只有回補才把 `sellAmt` / `costBasis` / `realized` 記進當年。
    否則年度報表的「總賣出金額」會被一筆尚未實現的交易灌水。回補也不計入 `buyAmt`——它是實現損益的
    成本項，不是進貨。
  - **超額回補**：回補股數超過空單時，多買的部分視為現股買進並發警告，而不是丟棄。
  - **欄位設為必填而非可選**：比照 `splitFeeTax` 的 `ticker`，一個可選的金額欄位在呼叫端忘記時會安靜地
    讀成 0，而忘掉的空頭腿是無聲的。編譯器抓出了四個需要補的合成 Holding。
- 💸 **借券費 0.08%**（`fees.ts`、`pnlEngine.ts`）— `BORROW_FEE_RATE` 定義在 `pnlEngine.ts`
  （放 `fees.ts` 會造成循環 import）。`calculateFee` 對 `SHORT` 賣出加收借券費，`splitFeeTax` 改為回傳
  第三個分量 `borrow`，`LedgerSummary` 新增 `feesBorrow`，維持 `fees = feesBrokerage + feesTax + feesBorrow`。
  **融券賣出一律付全額證交稅**——資券當沖不適用當沖減半，同日建倉又回補也一樣。
- 🐛 **當沖撮合會吃掉融券腿（本版新發現）** — 當沖候選清單在 fallback 分支會擴大成當日**全部**的反向交易。
  同日只要有一筆當沖買進、卻沒有對應的當沖賣出，融券賣出就會被當沖撮合掉，空單根本不會建立；反向亦然。
  候選過濾現在明確排除 `SHORT`，兩個回歸測試鎖住。
- 📊 **儀表板多空並列**（`holdingRows.ts`、`DashboardPage.tsx`）— `buildHoldingRows` 改為 `flatMap`，
  一檔同時有波段持股與空單時產出兩列（`rowKey` 相異），新增「方向」欄以紅「多」／綠「空」標示，
  空單股數以負數呈現。表尾新增三行小計：多頭市值（台股）／空頭市值（台股）／淨曝險（台股）。
  - **只加總台股**：把美股市值加進台股小計是無意義的數字。
  - **KPI 面板同步修正**：「持倉市值」原本會把空單的買回成本加進去，「投入總成本」則因為多空各一列而
    把同一檔的成本算兩次。兩者現在只取多頭列；未實現損益相反，多空相加才是總損益，維持逐列加總。
  - 超賣警告文案改為「發現 N 筆資料異常（如超賣、超額回補）：」——超額回補並沒有以持有股數為上限。
- 🧾 **交易表單與 CSV**（`TransactionForm.tsx`、`csv.ts`）— 交易性質新增「融券」；選取時自動改回全額稅率
  （不沿用當沖的減半），手續費自動試算納入借券費。**融券回補是買進**，既有的持股快選不會出現，因此新增
  對稱的「空單快選」清單（顯示未回補股數）。CSV 匯入接受「融券」與 `SHORT` 兩種寫法，匯出寫成「融券」。
- 🗄️ **Schema**（`schema.sql`）— `transactions_tx_nature_check` 允許 `'SHORT'`。
  **DDL 尚未對 DEV / PROD 執行**：在執行前，融券交易存檔會被資料庫的 CHECK 約束擋下。
- 🧪 **測試** — 95 檔 / **1518** 個測試 100% 通過（exit 0），新增 24 個；`npm run build` exit 0；
  `npx oxlint src` 0 error。兩輪 reviewer：Stage A PASS 零 findings，Stage B 找到上述兩個 KPI 加總缺陷。

### 0.9.28-dev.1（2026-09-02）— 現股當沖先賣後買撮合、波段持股獨立保護與端到端 E2E 驗證

- ⚡ **現股當沖先賣後買獨立撮合與損益精算**（`pnlEngine.ts`、`pnlEngine.test.ts`、`verify-daytrade-short-e2e.cjs`）—
  - 於 `computeLedger` 導入按交易日分群與「同日當沖優先撮合（Intraday Day-Trade Priority Matching）」機制。
  - **當沖先賣後買沖銷**：當日先賣出放空、盤中再買進回補時，系統將當沖買賣雙方獨立撮合結算，扣除買賣手續費與 0.15% 減半證交稅後精算已實現損益（`realized = 賣出淨額 - 買入總成本`），當日沖銷完畢後部位自動歸零，不再誤發超賣（Oversold）警告或將賣出成本視為 0。
  - **波段持股保護**：當使用者帳戶內已有既有長線持股時，當日的當沖先賣後買交易會優先在同日獨立撮合，完全不吃掉或均化先前的波段持股成本，原持股均價與未實現損益 100% 保持恆定，完全對齊券商對帳單行為。
  - **部分回補健全性**：若當沖賣出僅部分買回，已回補部分優先結算當沖損益，剩餘未回補部位維持常規部位與超賣處理。
- 🧪 **完整驗證與 Playwright E2E 測試** — 單元測試 95 檔 / 1490 個測試 100% 通過；新增 `verify-daytrade-short-e2e.cjs` 端到端真實瀏覽器模擬驗證 100% 通過；`npx oxlint` 0 error，`npm run build` exit 0。
