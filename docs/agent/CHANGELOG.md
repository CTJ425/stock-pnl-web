...
**發送時間全部下放各帳號，設定頁間距統一（Task 165 step 2g）**

- 管理後台移除「發送排程」設定：發送時間由各帳號在自己的 Discord 通知設定頁決定，後台只保留全域經濟快報 Webhook 與各帳號唯讀狀態清單。
- 全域頻道的發送時間維持 17:30／21:30，並繼續作為未自訂帳號的預設值；要調整改由資料庫端處理。
- 時間下拉的「跟隨全域」改為「預設（HH:MM）」——後台已無「全域」設定畫面，舊名稱會讓人找不到它在哪裡設定。
- Discord 通知設定頁的間距改為單一節奏：區塊內一律 12px、區塊之間 24px。原本網址輸入框與按鈕列之間是 0px，緊貼在一起。
- 三個發送時間下拉改為同一左緣對齊。

### 0.9.60-dev.3

**各帳號可自訂 Discord 發送時間（Task 165 step 2f）**

- 每個帳號可自行設定經濟快報（快報／完整版）與個人持股報告的發送時間，預設跟隨管理員排程。
- 管理後台的排程設定仍然存在，並成為未自訂帳號的預設時間。
- 管理後台各帳號表格調整欄寬與行距，改善可讀性。

### 0.9.60-dev.2

**Discord 設定介面統一（Task 165 step 2e Revision 1）**

- 所有帳號（含管理員）都使用同一個 Discord 通知設定頁面，不再有後台專屬的特權版介面。
- 經濟快報狀態改為兩行顯示，分別標示全域頻道與自己的頻道，避免「未設定」被誤解為收不到。
- 管理後台的各帳號區塊改為唯讀狀態清單；發送排程移到全域 Webhook 區塊下方。

### 0.9.60-dev.1

**Discord 推播設定下放到各帳號自助管理（Task 165 step 2e）**

- 新增使用者自助設定頁：每個登入帳號可自行設定經濟快報與個人持股兩支 Discord Webhook、各自啟用開關、各自推播測試，入口在右上角使用者選單。
- 經濟快報新增 `market_enabled` 開關，可暫停推播而不必清除已設定的網址。
- 自訂 Webhook 現在會同時收到快報與完整版兩份（原本只有完整版）。
- 管理員後台維持不變，仍保有全域經濟快報 Webhook 與各帳號設定的編輯權限，並新增各帳號的經濟快報啟用開關。
- 資料庫新增 `user_discord_settings.market_enabled` 欄位（含一次性 backfill）。
