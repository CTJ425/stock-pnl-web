...
### 0.9.29-dev.3（2026-09-03）— 觀察股票同產業自動群組聚合、膠囊篩選與分析頁選單分組

- 🏷️ **觀察股票同產業自動聚合群組（Auto-Grouping）**（`stockGrouping.ts`）—
  - 核心邏輯：依據個股解析之官方/推導產業別（`getStockCategory`），當同產業標的數 $\ge 2$ 時（例如加入長榮 2603 與陽明 2609 同為「航運業」），自動將其聚合成獨立產業族群（如「航運業 (2)」）；單一標的或未分類者歸於「其他」。
  - 產業名稱規範化（`getGroupCategoryName`）：將簡短徽章標籤（如「航運」、「半導體」）與官方產業全名（「航運業」、「半導體業」）無縫對齊，特別資產（ETF、ETN、特別股）保持原樣。
- 📱 **庫存總覽觀察股票分組檢視與快速篩選膠囊**（`WatchSection.tsx`、`index.css`）—
  - 膠囊列（Filter Chips）：於圖卡與條列模式頂部動態提供「全部 (N)」、「產業 (M)」、「其他 (K)」一鍵切換膠囊，快速聚焦特定族群。
  - 分組標題：圖卡模式下依群組呈現微型標籤與計數；條列模式下以群組列（`watchlist-group-row`）清楚劃分區塊。
  - 狀態保留：在圖卡與條列檢視切換時，所選篩選膠囊狀態無縫維持；當標的移除使得群組數量 $< 2$ 時，自動解構回復無分組佈局。
...
- 🗂️ **觀察股票緊湊型小卡（方案 A）**（`WatchSection.tsx`、`index.css`）— 網格改為 `repeat(auto-fill, minmax(165px, 1fr))`，卡片內距緊湊化（`10px 12px`），卡片高度壓縮至 ~72px（減少約 35%~40% 垂直佔位）。
  - 雙行佈局：第 1 行整合股票代號、名稱、微型分類標籤與右上角移除按鈕；第 2 行對齊現價（18px，Tabular Nums 與紅綠配色）與漲跌幅百分比。
  - 條列檢視同步在名稱旁展示微型分類徽章。
