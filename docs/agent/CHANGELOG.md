...
### 0.7.16-dev.1（2026-08-14）— 總體經濟「每日成交量」表格風格比照三大法人垂直矩陣重構＋相較前一日（DoD）增減熱力著色
...
- 📐 **總體經濟・「每日成交量」表格風格比照三大法人重構（Task 102）**：
  ...
  - **頁尾走勢與 Streak 下沉**：將 15 日 SVG 波折圖（`SparkCell`）與趨勢標籤（如 `連 N 日增量`、`連 N 日上漲`、`7日累計漲跌`）垂直下沉至 `tfoot` 各項統計欄位下方（`.tfoot-cum-trend`），與上方數據精準對齊。
  - **行動端體驗提升**：去除側邊合併欄後消除橫向空間佔據，在桌面、平板與手機端體驗完全一致。
