...
### 0.9.30-dev.6（2026-09-04）— 個股分析股票產業別顯示字級與清晰度優化

- 🏷️ **根除產業別太小、截斷與低對比模糊問題**（`QuoteTab.tsx`、`index.css`）—
  - 核心痛點：個股分析行情卡片中的產業別徽章原本複合使用 `.watchlist-card-badge`，受到 CSS 定義順序與樣式覆蓋影響，繼承了緊湊小卡的 `max-width: 48px`、`font-size: 9px` 與暗淡低對比色彩 `var(--ink-muted)`（淺色主題下對比度僅 2.8:1）。導致 8 個字長產業名稱（如「電腦及週邊設備業」）被截斷為省略號，繁體中文字筆畫模糊成團。
...
- 📌 **個股分析標題列產業標籤同步優化**（`index.css`）—
  - 針對 `.detail-title .badge` 進行專屬樣式強化，字級提升至 **12px**，文字色彩改用 `var(--ink)`，邊框使用 `var(--border-strong)`，與下方行情卡片產業標籤在層次、字級及清晰度上保持和諧統一。
- 📑 **個股下拉切換選單產業群組標題銳利化**（`index.css`）—
  - 將選單分組標題 `.hmenu-head`（如「觀察 ── 半導體業」）由淡灰 `--ink-muted` 提升為 `var(--ink-secondary)` 並加粗為 `font-weight: 600`，使下拉分組一目了然。


- 📐 **徹底消除持股表格橫向捲軸（零下方滑動）**（`index.css`）—
  - 改善痛點：先前版本字級與內距提升後，10 欄寬度撐至 1,175px，超過內容容器（1,140px），導致在所有常見桌面與筆電解析度（1280px、1366px、1440px、1920px）下，表格底部皆出現橫向滾動條，必須向右滑動才能看到完整報酬率與損益。
  - 零滑動黃金比例微調：
    - 主文字與數值單元格 `.data-table td`、`.data-table .num` 微調至 **13px**（介於原 12.5px 偏小與 14px 偏大之間的視覺甜蜜點）。
    - 表頭 `th` 定位為 **12px**，內距收斂至 `9px 7px`；主儲存格內距微調為 `9px 7px`，首末欄左右縮減至 `10px`。
    - 次級輔助文字（未含費、淨收、券商牌告）維持 **11px**，清晰不擁擠。
    - 表頭說明圖示 `th-head` 與 `th-plain` 內距緊湊化（右距 7px），釋放近 40px 欄位冗餘寬度。
  - 響應式自適應縮放：
    - 新增 `@media (max-width: 1120px)` 智慧緊湊階級（主字 12px、次字 10px、padding 8px 5px），讓即便是 1024px 螢幕或半螢幕多工也能 100% 完整容納，完全不出現下方滾動條。
- 🎨 **設計範本即時預覽頁面同步升級**（`docs/design/holdings-table-mockup.html`）—
  - 同步更新設計範本方案切換器，提供「✨ 黃金平衡款 (13px / 11px・零橫向滑動)」、「上一版微大款 (14px / 11.5px)」與「舊版原尺寸 (12.5px / 10.5px)」三版即時深淺色對比。

### 0.9.30-dev.4（2026-09-04）— 庫存總覽持股表格分組列小計移除與字級微升階梯優化

- 🗂️ **庫存總覽分組列回歸通欄純章節標題（移除市值與未實現損益小計）**（`DashboardPage.tsx`、`DashboardPage.test.tsx`）—
  - 改善痛點：庫存總覽上方已具備完整 `MarketPanel` Summary 數據卡片，表格分組列（多單・X 檔、空單・Y 檔）原本在後方欄位顯示的小計數字產生資訊重複與視覺干擾。
  - 改動：移除分組列後方之市值與損益欄位，將分組標題儲存格擴展為 `colSpan={10}` 橫跨全表格寬度，建立清晰的章節視覺層次。
- 🔍 **字級微升階梯調整與次級樣式統整**（`index.css`、`DashboardPage.tsx`）—
  - 核心痛點：原主字級偏小（數值 12.5px、次級 10.5px）導致高解析度螢幕辨識吃力，但若盲目放大又會造成 10 欄寬表格橫向溢出。
  - 黃金階梯縮放：
    - 表頭 `th` 由 12px 微調至 13px。
    - 主數值 `.num`、第一行數值、代號與股名由 12.5px / 13.5px 統一提升至 14px。
    - 次級文字（未含費、淨收、券商備註）由 10.5px 提升至 11.5px，移除 JSX 中硬編碼之 inline `fontSize: 11`，全權由 CSS 階層精準控管。
    - 表格單元格 padding 微調為 `11px 9px`，有效提升垂直行間呼吸感，維持水平不溢出。

### 0.9.30-dev.3（2026-09-03）— 未實現損益金額同步雙行直顯券商 APP 牌告口徑

- 💰 **未實現損益金額雙行直顯與券商手續費折讓差額對齊**（`holdingRows.ts`、`DashboardPage.tsx`、`QuoteTab.tsx`、`reportProxy.ts`）—
  - 核心痛點：使用者回報聯電（2303）持股損益手機 APP 顯示 `-10,610` 元，而網頁端顯示 `-10,485` 元，精準相差 125 元。經查即為現價 125 元（市值 125,000 元）下，券商牌告原價手續費（178 元）與 3 折實質手續費（53 元）之 125 元折讓退佣差額。
  - 雙行直顯（零懸停一眼全覽）：
    - 庫存總覽「未實現損益」欄位：主標（粗體）顯示實質淨損益（如 `-NT$10,485`），副標（11px 灰字）直顯 `券商 -NT$10,610`，與「未實現報酬率」欄位完美呼應成雙成對。
    - 個股分析持股概況（`QuoteTab.tsx`）同步在未實現損益金額後方直顯 `(券商 -NT$XX)`。

### 0.9.30-dev.2（2026-09-03）— 未實現報酬率雙行直顯券商 APP 牌告口徑 (方案 1)

- 📊 **未實現報酬率雙行直顯與券商 APP 月退制口徑對齊**（`holdingRows.ts`、`DashboardPage.tsx`、`QuoteTab.tsx`、`pnlEngine.ts`）—
  - 核心痛點：台灣多數券商 APP（如國泰、永豐、富邦、元大等）庫存頁採「月退制」，盤中與未沖銷明細先按牌告原價（0.1425%）預扣賣出手續費，次月才退佣；而本專案預設採折讓後實質費率（如 3 折 0.04275%），產生剛好約 0.10% 的退佣差額（例如聯電 2303 手機顯示 -7.86%，專案顯示 -7.76%）。
  - 雙行直顯（零懸停一眼全覽）：
    - 主標（粗體）：實質淨報酬率（反映使用者設定之折讓，落袋為安真實數據）。
    - 副標（11px 次要灰）：`券商 {brokerRoi}`，依台灣法定牌告 0.1425% 預扣精算，1:1 完全吻合手機券商 APP 畫面。
    - 自適應防呆：若工作區未設折讓（牌告原價），副標自動隱藏，維持畫面乾淨。
  - 個股分析持股概況同步支援：`QuoteTab` 持股資訊區塊同步在報酬率後方標註 `(券商 XX%)`。

### 0.9.30-dev.1（2026-09-03）— 觀察股票卡片迷你極簡化微調

- 📐 **觀察股票卡片迷你緊湊化微調**（`index.css`）—
  - 網格寬度進一步收縮至 `minmax(136px, 1fr)`（自 165px 下調約 17.5%），讓同列可排列更多標的。
  - 卡片高度由 72px 壓至 58px（減少約 20% 高度），內距微調至 `7px 8px`，圓角微調為 6px。
  - 字級與間距精準縮放：現價調整為 15px，代碼與股名微調為 11.5px，微型徽章調整為 9px（max-width 48px），移除按鈕調整為 16x16px，保留紅綠漲跌配色與等寬數字排版。
