...
### 0.9.68-dev.3（2026-09-24）— 手機版「新增交易」改排在工作區選單右邊

- 📱 **手機頁首順序**：寬度 ≤ 720 px 時，「新增交易」圖示按鈕從 logo 旁邊移到工作區選單與帳號之間，不再像是品牌圖示的一部分；電腦版不變。
- ✅ **驗證**：Playwright 在 1440／1024／375／320 px 確認頁首無溢出、頁面無橫向捲動；手機捲到底時版本號在底部導覽列上方 31 px，版本號不斷行；375 px 點「新增交易」會開啟新增視窗。`npm test` 2,459 通過、7 略過、0 失敗；`npm run build` exit 0。

### 0.9.68-dev.2（2026-09-24）— 版本號移到頁尾免責聲明後方

- 🏷️ **版本號改放在頁尾**：左下角固定的版本徽章拿掉，改接在頁尾免責聲明最後（「…諮詢之用 · 0.9.68」），字色比聲明再淡一階且不會斷行。電腦版不再蓋住左下角內容；手機版不必為徽章多留一列，頁尾直接負責讓出底部導覽列的空間。登入頁不再顯示版本號。
- ✅ **驗證**：142 個測試檔／2,466 條測試：2,459 通過、7 略過、0 失敗；`npm run build`、`npm run lint`、`npm run typecheck:edge` exit 0。

### 0.9.68-dev.1（2026-09-24）— 移除 AI 分析、「新增交易」移到頁首、Discord 持股卡片加上券商口徑（Task 167）

- 🗑️ **移除 AI 分析**：個股頁「AI 分析」分頁與管理後台「AI 設定」一併拿掉，相關前端程式與測試全數刪除；`ai-proxy` Edge Function 移除，資料庫刪除 `app_settings` 資料表與 `get_ai_settings()`。日後要重做可從 git 歷史找回。
- ➕ **「新增交易」移到頁首**：右下角浮動按鈕改為頁首按鈕，不再蓋住右下角的資訊；手機寬度只顯示圖示。
- 📒 **Discord 持股卡片加上「券商」**：與庫存總覽的「券商」欄同一算法（台股牌告 0.1425%、不打折）。總計列顯示「券商 ±金額」，每檔持股在未實現後加「（券商 ±金額）」；數字與實際未實現相同時不重複顯示，美股不適用。頁尾註明「券商＝牌告 0.1425% 未折讓」。
- ✅ **驗證**：142 個測試檔／2,466 條測試：2,459 通過、7 略過、0 失敗；`npm run build`、`npm run lint`、`npm run typecheck:edge` exit 0。
