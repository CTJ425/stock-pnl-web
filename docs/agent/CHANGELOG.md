...
### 0.7.16-dev.1（2026-08-14）— 總體經濟「每日成交量」與個股籌碼「三大法人買賣超」表格風格比照總體經濟三大法人垂直矩陣重構

- 📐 **個股分析・籌碼「三大法人買賣超」表格風格比照總經重構（Task 104）**：
  - **表格結構收斂**：移除最右側寬度 220px 之「走勢」合併欄位（`col-trend-rowspan`），收斂為 6 個清晰欄位（`日期`、`外資（不含自營）`、`外資自營商`、`投信`、`自營商`、`三大法人合計`）。
  - **頁尾走勢與 Streak 下沉**：將歷史 SVG 波折圖（`SparkCell`）與「連 N 買 / 連 N 賣」標籤垂直下沉至 `tfoot` 各法人累計統計欄位下方（`.tfoot-cum-trend`），與上方數據精準對齊。
  - **測試與驗證同步**：以 TDD 流程更新 `StockDetailPage.test.tsx`，34 項個股分析單元測試與全專案 63 檔單元測試（951 測試）100% 通過。

- 📐 **總體經濟・「每日成交量」表格風格比照三大法人重構（Task 102）**：
  - **表格結構收斂**：移除最右側寬度 220px 之「近 15 日走勢」合併欄位（`col-trend-rowspan`），收斂為 6 個清晰欄位（`日期`、`成交金額（億元）`、`成交股數（億股）`、`成交筆數（萬筆）`、`加權指數`、`指數漲跌`）。
  - **相較前一交易日（DoD）增減熱力著色**：
    - 成交金額、成交股數、成交筆數與加權指數各欄位依據「相較前一交易日（`days[i - 1]`）」之增減幅度，套用紅綠色彩（增量／上漲為紅，縮量／下跌為綠）與相對強度熱力底色（`heatStyle`）。
    - 指數漲跌欄位維持正點數紅、負點數綠與單日強度熱力底色。
  - **頁尾走勢與 Streak 下沉**：將 15 日 SVG 波折圖（`SparkCell`）與趨勢標籤（如 `連 N 日增量`、`連 N 日增筆`、`連 N 日上漲`、`7日累計漲跌`）垂直下沉至 `tfoot` 各項統計欄位下方（`.tfoot-cum-trend`），與上方數據精準對齊。
  - **測試與驗證同步**：更新 `TwMarketSection.test.tsx` 與 `verify-macro-turnover.cjs`，所有 63 檔單元測試（951 測試）與跨裝置 E2E 截圖驗證 100% 通過。
