...
### 0.9.67-dev.1（2026-09-23）— 自選股產業分組不再來回跳動（BUG-085）
...
- ✅ **驗證**：151 個測試檔／2,620 條測試：2,613 通過、7 略過、0 失敗；`npm run build` exit 0。DEV 已套用 DDL 並重新部署 `stock-price`，連續兩次查詢 8150 皆回傳「半導體業」（第二次為快取命中），`verify_setup()` 全數 PASS。
