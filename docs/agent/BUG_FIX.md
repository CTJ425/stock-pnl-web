...
- Agent: Claude
- Status: ACTIVE
- Timestamp: 2026-09-05 (九項深度稽核發現已逐條查證)
...
- **查證（2026-09-05，主 session 逐行讀碼）**: ✅ **屬實**。`csv.ts` 匯出處 `const { fee, tax } = splitFeeTax(tx)` 只解構兩個欄位、丟棄 `borrow`，CSV 表頭也沒有借券費欄位；匯入的 splitMode 分支 `feeTax = fee + tax`。匯出再匯入一趟，融券賣出的 `fee_tax` 縮水，實收金額被高估，已實現損益虛增且無任何警告。
- **Status**: OPEN
...
- **Severity**: P0（融券帳務損毀）
- **查證（2026-09-05，主 session 逐行讀碼）**: ✅ **屬實，兩半皆成立**。`fees.ts` 的 `calculateFee` 內有 `if (input.nature === 'SHORT') fee += floorSafe(amount * BORROW_FEE_RATE)`，而 `proposeFeeCorrections` 呼叫時確實沒有傳 `nature`；`RecalcFeesModal` 的 `updateTransaction` 參數中也確實沒有 `fee_rate`（同檔的 `StockSplitModal` 有傳，可見是遺漏而非設計）。
- **Status**: OPEN
...
- **Severity**: P0（計算錯誤）
- **查證（2026-09-05，主 session 逐行讀碼）**: ✅ **屬實**。`buyTickers` 與 `matchingTxs` 兩處都只用 `tx.tx_type === 'BUY'` 篩選，未排除 `tx_nature === 'SHORT'`；全檔唯一一次出現 `tx_nature` 是 `updateTransaction` 的原樣帶過。融券回補會被當成現股買進換算，而未平倉的融券賣出腿完全不在轉換範圍內。
- **Status**: OPEN
...
- **查證（2026-09-05，`route:reviewer` 逐處核對）**: ⚠️ **7 處中 6 處成立，第 5 處成因判斷錯誤，且證據引用了錯誤的檔案**。
  - **成因更正（第 5 處 `listUsers`）**: 截斷風險真實，但成因是 Supabase Auth（GoTrue）Admin API 自己的 `page`/`perPage` 分頁，**與 PostgREST `max_rows` 無關** —— 把 `max_rows` 調大並不會修好它。需要使用者總數超過 1000 才觸發，以目前 PROD 2 名使用者而言屬遠期風險。
  - **證據更正**: 原主張引 `sources/supabase/config.toml` 證明雲端行為，但該檔開頭註解指向 local-development 文件，管的是 `supabase start` 的本機堆疊，**不是兩個雲端專案**。雲端的 Max Rows 是專案 API 設定（Supabase 文件記載預設同為 1000，故結論很可能仍成立），但兩個專案的實際值尚未查證。
  - **第 7 處語意更正**: 被截斷的是 `.select()` 回傳的列數，不是寫入本身 —— UPSERT 會把 `toInsert` 全部寫入，但只回報 1000 筆，管理者會誤判還原不完整而重複操作。
  - **優先度更正**: 第 1 處 `listTransactions` 的門檻是「單一 workspace 的交易筆數」，與使用者人數無關，一名活躍交易者數年即可觸及；一旦觸發，損益與庫存會依不完整的交易序列計算且完全無聲。**這一處的性質是靜默金額錯誤，實質應視為 P0 而非 P1。**
- **Status**: OPEN
...
- **Severity**: P1（功能阻斷）
- **查證（2026-09-05，`route:reviewer`）**: ✅ **屬實**。`pnlEngine` 對每個 `key` 只產生一個 `Holding`，而其 filter 是 `pos.qty > 0 || pos.shortQty > 0`（OR），所以同一檔可同時有多頭與空單；`buildHoldingRows` 於兩者皆成立時先 push LONG、後 push SHORT，兩列共用同一個 `holding` 參照。`AnalysisPage` 用 `r.holding.key` 而非唯一的 `r.rowKey`，於是 `.find()` 永遠命中先被 push 的 LONG 那筆。實例：同時持有 2330 現股 1000 股與融券空單 500 股時，空單項目點了也切不過去。既有測試完全未涵蓋「同一檔同時有多空」的情境。
- **Status**: OPEN
...
- **Severity**: P1（功能異常）
- **查證（2026-09-05，`route:reviewer`）**: ⚠️ **主體屬實，但「鎖死」誇大**。純融券部位的 `qty`／`avgCost`／`rawAvgCost` 在 `pnlEngine` 中確實算成 `0` 而非 `null`，於是 `WhatIfTab` 的 `isHeld = rawAvgCost !== null` 判為 true 卻代入 0，`whatIf()` 的 `qty > 0` 與 `buyPrice > 0` 兩個守門同時失敗，初次進入畫面是空的 —— 比「未持有」時的預設（股數 1、買進價＝現價）還糟。**但兩個輸入框都沒有 `disabled`**，使用者手動輸入非零值後即恢復正常運算。應描述為「初始預設值失真、首屏空白」，不是永久無法操作。既有測試未涵蓋純融券情境。
- **Status**: OPEN
...
- **Severity**: P1（可用性風險）
- **查證（2026-09-05，主 session 逐行讀碼）**: ✅ **屬實，但措辭需修正，且本專案自行讓它變得可觸發**。
  - **措辭更正**: 該上傳並非「無條件」，它在 `if (regenerate)` 區塊內。真正缺的是 `generated > 0` 守衛。
  - **可觸發性由 BUG-053 引入**: 在 0.9.32 的 AUDIT-12／BUG-053 修正之前，`assembleOne` 拋錯會讓例外衝出整個迴圈，`manifest.json` 根本不會被寫入，manifest 因而停在前一天。加上逐筆 `try/catch` 之後，迴圈會吞掉每一筆錯誤並跑完，於是在全數失敗時仍把 manifest 推進到一個沒有任何報告檔的 ymd。該修正本身仍是對的（整輪中止更糟），但它把本缺陷從不可達變成可達，**優先度應相應提高**。
- **Status**: OPEN
...
- **查證（2026-09-05，主 session 逐行讀碼）**: ✅ **屬實**。`backup-transactions/index.ts` 的 `assertCronSecret` 仍是 `if (!expected || got !== expected)`。附帶一項待修的文件債：該函式上方註解寫「Same shape as stock-report/index.ts:848」，但 stock-report 的對應函式已於 0.9.32 改用 `secretsMatch`，且實際位置是 901 行 —— 註解的內容與行號都已過時。
- **Status**: OPEN
...
- **查證（2026-09-05，`route:reviewer`）**: ✅ **屬實，兩半皆獨立成立**。(a) 同一筆賣出交易可被 push 兩次 `SellDetail`：當沖配對迴圈對股數大於單筆可配對量的賣出會多次 push 同一個 `txId`，其剩餘量還會落入一般賣出分支再 push 一次。**用既有測試 `pnlEngine.test.ts` 的資料（賣 1000 對買 600，皆 `DAY_TRADE`）即可直接重現** —— 該測試只斷言 `warnings` 與 `realizedTw`，未對 `yt.sells` 的 `txId` 唯一性做任何斷言。(b) `SellDetail` 介面根本沒有欄位可區分「現股賣出」與「融券回補」，兩個分支都 push 進同一個 `yt.sells`，`YearlyPage` 只能硬編碼「賣出」，因此融券回補（買進動作）顯示為賣出，方向相反。
- **Status**: OPEN
