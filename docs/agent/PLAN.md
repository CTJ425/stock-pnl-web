...
**不用季報 EPS（`t187ap06_L`）**：季更頻率太低、欄位解析繁瑣，對「盤後看一眼」的使用情境
邊際效益低。日後要加再說。
...
月營收 API 只回「最新月份」。要讓 AI 看得出趨勢就得有歷史，但又不想為此開資料表，
所以 `mergeRevenueMonths()` 每晚把最新月份併進既有檔（依年月去重、上限 12 個月）。
代價：首次執行只有 1 筆，要累積一年才滿。這比為了歷史而開一張新表划算。
