...
## ✅ 已完成：基本面（EPS 與估值指標）v0.3.7-dev.5

- Agent: Claude
- Status: IMPLEMENTED — 程式碼、測試、dev Supabase 建表與部署皆已完成並實測
- Timestamp: 2026-07-25 22:30:00 Asia/Taipei

起因是使用者問「想加上 EPS，這個有現成的還是要用法人的去算」。
**答案：不能用法人算** —— 法人是交易流量（誰買了幾股），EPS 是公司獲利 ÷ 股數，兩者無關。

### M. 資料源實測結果（勿再重新推導）

| 來源 | 內容 | 關鍵限制 |
| --- | --- | --- |
| `opendata/t187ap06_L_{ci,fh,ins,bd,mim}` | **基本每股盈餘（元）**、營業收入、淨利歸屬母公司業主 | **按產業分五表，必須全抓合併**；**無季別參數**只回最新一期 |
| `exchangeReport/BWIBBU_ALL` | 本益比、殖利率、股價淨值比 | **沒有 EPS 欄位**；無 date 參數 |
| `exchangeReport/STOCK_DAY_AVG_ALL` | 收盤價 | 用來反推年化 EPS＝收盤價 ÷ 本益比 |

實測筆數：`ci` 1044 / `fh` 13 / `ins` 6 / `bd` 3 / `mim` 4 = **1070 檔**（與實際入庫列數吻合）。
`_basi`（金融業）實測抓取失敗，視為可選，缺它不影響上市主要標的。

**2330 fixture**（已寫成單元測試）：民國 115 年第 1 季、`基本每股盈餘（元）` 22.08、
營業收入 1,134,103,440 千元、淨利歸屬母公司業主 572,479,752 千元。
BWIBBU 同日：`PEratio 31.59`、`DividendYield 0.94`、`PBratio 10.34`；收盤 2350 → 反推 TTM EPS 74.39。

**交叉驗證**（反推年化 vs 財報單季，數值互相合理）：
2330 74.39/22.08、2303 3.98/1.29、2609 3.08/0.41、1802 0.13/0.20（PE 392 倍 → 反推極不精確）。
**0050 在 BWIBBU 與財報五表都查無** —— ETF 本質上沒有 EPS。

### N. 三個刻意的設計決定

1. **獨立分頁而非併進籌碼**：EPS 每季、籌碼每日。混在一起會讓「盤後籌碼」報告與其 PDF 失焦、
   資料日期語意混亂。PDF 維持只含籌碼。
2. **`stock_fundamentals` 刻意不設保留期**，與 `chip_raw_cache` 的 7 天 prune 是相反取捨：
   那是每天數 MB 的原始檔快取，這是「一檔一季一列」的極小長期事實（1000 檔 × 4 季 ≈ 4000 列/年）。
   **保留歷史是 EPS 逐季趨勢的唯一來源** —— 官方端點給不出歷史。
3. **前端 schema 守門由 `=== 2` 放寬為 `>= 2`**：基本面是加法。若因一份 schema 2 報告缺 fundamentals
   就判為未命中，會讓所有人白走一天的慢路徑（即點即產約 8 秒）。

### O. 抓取頻率：每季只抓一次

`expectedLatestQuarter(now)` 依台灣財報申報期限（Q1 5/15、Q2 8/14、Q3 11/14、年報 3/31）
推算「此刻應已公布的最新一季」，已在庫就**完全不發外部請求**。五表合計約 7.5MB，天天抓純屬浪費。
**實測有效**：第二次 `generate-all` 由 10.1 秒降到 1.9 秒。
只有盤後批次會同步財報；即點即產不同步（會逾時），只讀已累積的季別。

### P. 發現的驗證陷阱（下次別再踩）

- **Storage public URL 有 Cloudflare 快取**（`max-age=3600`）。批次跑完後用
  `/object/public/reports/...` 檢查，可能拿到最多 1 小時前的舊檔（header `cf-cache-status: HIT`）。
  實測 0050 的 public 路徑回舊的 schema 2，而前端實際用的
  `/object/reports/...`（`supabase.storage.download()`，帶 anon key）回正確的 schema 3。
  **前端不受影響**，但驗證必須改用後者。
- **臨時 harness 的清理要用絕對路徑**：本輪兩次 `rm -f` 在 repo root 執行（shell cwd 被重置），
  `-f` 靜默成功、`ls` 也在同一目錄回報找不到，誤判已刪除，實際殘檔還在 `sources/`（被 lint 抓到才發現）。

### Q. 下一步

- **EPS 逐季趨勢**：目前只有 1 季，2026-08-14 起 Q2 公布後會自動累積，UI 到 2 季就會出現走勢圖。
  不需要再改程式碼。
- **技術面仍是佔位頁**：需先解 §G 的 `price_daily` 儲存問題（約 400 天保留期）。
