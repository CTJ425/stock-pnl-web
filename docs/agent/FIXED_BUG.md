...
### Bug ID: BUG-010 — 全市場法人準時到手，卻被畫到軸外並判成「延遲」
- **Date**: 2026-08-05（0.6.33 引入，0.6.36-dev.2 修復）
- **Discovered by**: 使用者看畫面回報「16:00 這班車啟動後，狀態都還是舊的，這樣是 BUG 嗎」
- **Symptom**: 後台時間軸標題停在「台股盤後・2026-08-04 這一輪」，
  而「三大法人・全市場」那一列的資料日已經是 2026-08-05 —— 同一條軸自相矛盾。
  該列同時被標成「延遲」並貼在軸的最右端。
- **證據**（2026-08-05 測試區實測）:
  `market/daily.json` 的 `asOf = 2026-08-05T08:00:04Z`（台北 16:00:04）、最後一筆 `date = 2026-08-05`；
  而 `chip.dataDate` 仍是 `2026-08-04`（T86 要到 16:30 那輪才到手，`batch_run_log` 的
  16:00 / 16:15 兩列 `t86_today = false` 可證，16:30 那列才轉 `true`）。
  以 8/4 15:00 為原點時 `hoursFromBase` 得 **25 小時**，`tlPercent(25)` = 131% 被夾成 100%，
  `judgeSource` 因 25 > `dueBy 3 + 0.25` 判成 `late`。
- **Root Cause**: 時間軸的基準日取自單一來源（`chip.ymd` / `chip.dataDate`，個股籌碼報告），
  但軸上五列來自不同批次：個股 T86 走盤後批次（16:30），
  全市場 BFI82U 走獨立排程 `market-daily`（16:00）。跑得快的那列必然跨出本輪座標。
- **Impact**: 每個交易日 16:00–16:30 之間，準時到手的全市場那列固定亮紅燈。
  永遠亮著的告警等於沒有告警 —— 正好違反 `timeline.ts` 自己立下的原則。
  資料本身完全正確，純顯示層問題。
- **Fix**: `timeline.ts` 新增 `roundBaseYmd()`，基準日改取**各來源資料日的最大值**；
  標題與軸座標共用同一個基準。是否屬於本輪改用「時間戳落不落在軸範圍 [0, TL_SPAN_HOURS]」判定，
  而不是比對 date —— 因為 **借券自報的是公布日（次一交易日）**，天生比本輪多一天，
  拿它比對會讓唯一該亮紅燈的那列反而被當成沒抓到；同理它也不參與 `roundBaseYmd` 的計算。
- **Tests**: `timeline.test.ts` 4 筆（含直接鎖住「25 小時 → late」這個舊行為的對照），
  `AdminStatusPage.test.tsx` 1 筆（全市場先到手時標題跟著跳輪、個股顯示等待中且不顯示舊日期）。
- **Status**: ✅ FIXED（0.6.36-dev.2）；⚠️ 尚未部署到正式區（純前端，隨 main 合併上線）
