...
name: Carbon (stock-pnl-web)
slug: carbon-stock-pnl
source-url: none
created: 2026-09-09
updated: 2026-09-09
notes: IBM Carbon Design tokens; White theme light, Gray 100 dark; IBM Plex type; square corners
-->
...
This skin is IBM Carbon Design — Carbon gray 10 paper, Carbon gray 100 ink, Carbon Blue 60 accent, Carbon gray 70 muted, IBM Plex type, square corners. Swap these values (or run [`onboarding.md`](onboarding.md)) and every new diagram inherits the new skin without touching any type-specific logic.
...
Every token is referred to by **semantic role**, not by its hex value. Type references (`type-*.md`) and SKILL.md say `accent`, not `#0f62fe`.

| Role | Purpose | Default (light) | Default (dark) |
|---|---|---|---|
| `paper` | Page background, default node fill | `#f4f4f4` (Carbon gray 10) | `#161616` (Carbon gray 100) |
| `paper-2` | Diagram container bg, secondary fill | `#e0e0e0` | `#262626` |
| `ink` | Primary text, primary stroke | `#161616` (Carbon gray 100) | `#f4f4f4` (Carbon gray 10) |
| `muted` | Secondary text, default arrow stroke | `#525252` (Carbon gray 70) | `#c6c6c6` (Carbon gray 30) |
| `soft` | Sublabels, boundary labels | `#8d8d8d` (gray 50) | `#8d8d8d` (gray 50) |
| `rule` | Hairline borders | `rgba(22,22,22,0.12)` | `rgba(244,244,244,0.12)` |
| `rule-solid` | Stronger borders, baselines | `#e0e0e0` (gray 20) | `rgba(198,198,198,0.25)` |
| `accent` | Focal / 1–2 max per diagram | `#0f62fe` (Carbon Blue 60) | `#78a9ff` |
| `accent-tint` | Fill for accent-bordered boxes | `rgba(15,98,254,0.08)` | `rgba(120,169,255,0.10)` |
| `link` | HTTP/API calls, external arrows | `#005d5d` | `#33b1ff` |

> **Brand palette source:** this skin maps to IBM Carbon Design. Light values come from the Carbon White theme, dark values from Carbon Gray 100. `paper` is `$layer-01` (gray 10 / gray 90-ish), `ink` is `$text-primary` (gray 100 / gray 10), `muted` is `$text-secondary`, `soft` is `$text-helper` (gray 50), `accent` is `$button-primary` Blue 60 `#0f62fe`, and `link` is Teal 70 `#005d5d` so an external call stays distinct from the focal blue. Carbon draws productive components with square corners, so every radius token below is `0`.
...
Any `rgba(22,22,22, X)` in light becomes `rgba(244,244,244, X)` in dark. Same opacities, RGB flipped. The accent gets a slight hue-shift brighter to read on dark paper.
...
| `terminal-ink` | `#f4f4f4` | Primary text, primary stroke (same Carbon gray 10 as default `ink`) |
...
|---|---|---|---|---|
| `title` | IBM Plex Serif | 1.75rem | 400 | Page H1 |
| `node-name` | IBM Plex Sans (sans) | 12px | 600 | Human-readable labels |
...
| `callout` | IBM Plex Serif *italic* | 14px | 400 | Editorial asides only |

### Font stack

```html
<link href="https://fonts.googleapis.com/css2?family=IBM+Plex+Serif:ital,wght@0,400;1,400&family=IBM+Plex+Sans:wght@400;500;600&family=IBM+Plex+Mono:wght@400;500;600&family=Noto+Sans+KR:wght@400;500;600&family=Noto+Serif+KR:wght@400&family=Noto+Sans+TC:wght@400;500;600&family=Noto+Serif+TC:wght@400&display=swap" rel="stylesheet">
```

### Korean labels

IBM Plex Sans and IBM Plex Serif carry no Hangul. A Korean `<text>` element extends its own family — never swap the skin:

```svg
<text font-family="'IBM Plex Sans', 'Noto Sans KR', 'Apple SD Gothic Neo', 'Malgun Gothic', sans-serif">결제 서비스</text>
```

Both Noto faces ship in the font link above, so the web font resolves before any locally installed one and the same file renders identically on macOS, Windows, and a reviewer's browser. The local families follow it for offline viewing. Page titles need the serif equivalent — `'IBM Plex Serif', 'Noto Serif KR', serif` — or a mixed Latin/Korean title resolves Hangul through the platform's generic serif and the two halves disagree. Google's `css2` endpoint slices Korean by unicode-range, so a diagram with a handful of Korean labels downloads only the slices it touches. The four templates carry both faces because a new diagram may contain Hangul; the shipped Latin-only examples keep the shorter link, since a file with no Hangul has nothing to resolve.
...
- **Sublabels stay Latin.** Ports, protocols, field types, and URLs are Latin anyway — keep `IBM Plex Mono` there and don't translate them. Hangul in a 9px mono sublabel is unreadable and has no mono face to fall back to.
- **Floor of 12px.** Hangul goes muddy below 12px. If a Korean name doesn't fit at 12px, cut the name — don't shrink the type.
- **Arrow labels, eyebrows, and legend text switch register.** Those slots are 7–8px IBM Plex Mono, uppercase and tracked, which Hangul has neither a face nor legibility for. A Korean label in one of those slots becomes 12px sans at weight 500 with no tracking and no uppercase transform, and its mask rect grows to match (16px tall, width from the budget above, still rounded to a multiple of 4). Latin labels in the same diagram keep the mono treatment.

**Load-bearing rule:** Mono is for *technical* content (ports, commands, URLs, field types). Names go in IBM Plex Sans sans. Page title is IBM Plex Serif. Italic IBM Plex Serif is reserved for annotation callouts (see [primitive-annotation.md](primitive-annotation.md)). **Never JetBrains Mono** as a blanket "dev" font.

### Traditional Chinese labels

IBM Plex Sans and IBM Plex Serif carry no Han. A Traditional Chinese `<text>` element extends its own family — never swap the skin:

```svg
<text font-family="'IBM Plex Sans', 'Noto Sans TC', 'PingFang TC', 'Microsoft JhengHei', sans-serif">請求項比對</text>
```

Both Noto TC faces ship in the font link above, so the web font resolves before any locally installed one and the same file renders identically on macOS, Windows, and a reviewer's browser. The local families follow it for offline viewing. Page titles need the serif equivalent — `'IBM Plex Serif', 'Noto Serif TC', serif` — or a mixed Latin/Han title resolves Han through the platform's generic serif and the two halves disagree. Google's `css2` endpoint slices Chinese by unicode-range, so a diagram with a handful of Chinese labels downloads only the slices it touches.
...
- **Arrow labels, eyebrows, and legend text switch register.** Those slots are 7–8px IBM Plex Mono, uppercase and tracked, which Han has neither a face nor legibility for. A Chinese label in one of those slots becomes 12px sans at weight 500 with no tracking and no uppercase transform, and its mask rect grows to match (16px tall, width from the budget above, still rounded to a multiple of 4). Latin labels in the same diagram keep the mono treatment.
...
| `radius-sm` | `0` | Small tags (Carbon: square) |
| `radius-md` | `0` | Node boxes (Carbon: square) |
| `radius-lg` | `0` | Containers, rings (Carbon: square) |
...
- **Serif + sans + mono**: three families, not more. If brand typography is all sans, keep IBM Plex Serif for `title` and `callout` anyway — the contrast is load-bearing.
