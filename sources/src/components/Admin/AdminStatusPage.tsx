...
export function AdminStatusPage() {
  ...
  const chain = useMemo(() => {
    ...
    return TW_CHAIN.map((spec) => {
      // 日 K 線與新聞沒有 sources 條目，改用各自檔案的涵蓋數推斷
      let stamp: SourceStamp | null = null
      let partial = false
      let cover: string | null = null
      if (spec.id === 'institutional' || spec.id === 'margin' || spec.id === 'borrow') {
        stamp = src?.[spec.id] ?? null
      } else if (spec.id === 'daily') {
        const n = data.coverage.daily ?? 0
        stamp = src?.institutional ?? null // 同一輪批次寫入，時間戳共用
        partial = held > 0 && n < held
        cover = held > 0 ? `${n} / ${held} 檔` : null
      } else {
        const n = data.coverage.news ?? 0
        stamp = { date: null, fetchedAt: data.manifest?.generatedAt ?? null }
        partial = held > 0 && n < held
        cover = held > 0 ? `${n} / ${held} 檔` : null
      }
  ...
  return (
    <>
      ...
      <div className="section glass" style={{ padding: '18px 20px' }}>
        <div className="rpt-section-head">
          <h3 className="head-tight">美國總體經濟</h3>
          <span className="source-tag section-stamp">
      ...
      <div className="section glass" style={{ padding: '18px 20px' }}>
        ...
        <div className="kpi-grid" style={{ marginTop: 14 }}>
          ...
          <div className="glass kpi">
            <div className="kpi-label">新聞檔</div>
            <div className="kpi-value">
              {data.coverage.news ?? 0}
              <span className="ast-unit"> / {data.coverage.held ?? 0}</span>
            </div>
            <div className="kpi-sub">來源查無結果時會缺檔</div>
          </div>
        </div>
