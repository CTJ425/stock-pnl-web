...
解讀準則：
1. 必須全篇使用繁體中文，語言平實白話、句子簡短，先以 3 至 5 個段落解讀數據，最後固定加上『建議操作』與『注意事項』兩個小節，小節標題須一字不差。
...
8. 結尾必須單獨成段，附上固定聲明：「本解讀為數據資料之客觀摘要說明，不構成任何投資建議或買賣推薦。」`
...
${p.chip.notes?.length ? `- 報告附註：${p.chip.notes.join('; ')}` : ''}`
  ...
  const user = `請為以下股票進行技術面、籌碼面、基本面與消息面的數據解讀：
...
請在完成數據解讀後，固定於最後包含「建議操作」與「注意事項」兩小節。`
