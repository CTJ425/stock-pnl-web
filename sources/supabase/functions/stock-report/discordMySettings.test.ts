...
function fake(over: Record<string, unknown> = {}) {
  const d = {
    ...
    readMarket: vi.fn(async () => ({ enabled: false, webhookUrl: null as string | null })),
...
describe('runMySettingsOp — status', () => {
  it('reports both blocks by last4 and never returns a whole URL', async () => {
    const d = fake({
      readMarket: vi.fn(async () => ({ enabled: true, webhookUrl: MARKET_HOOK })),
      readSettings: vi.fn(async () => ({ enabled: true, webhookUrl: HOLD_HOOK })),
    })
    const result = ok(await runMySettingsOp(d, ME, { op: 'get' }))
    expect(result.status.market).toEqual({ enabled: true, configured: true, last4: 'Mmmm' })
  ...
  it('reports an unconfigured account as inheriting the global channel', async () => {
    ...
    expect(result.status.market).toEqual({ enabled: false, configured: false, last4: null })
  })
...
describe('runMySettingsOp — 經濟快報', () => {
  ...
  it('switches on once a URL is stored', async () => {
    const d = fake({ readMarket: vi.fn(async () => ({ enabled: false, webhookUrl: MARKET_HOOK })) })
  ...
  it('switches off when the URL is cleared', async () => {
    const d = fake({ readMarket: vi.fn(async () => ({ enabled: true, webhookUrl: MARKET_HOOK })) })
